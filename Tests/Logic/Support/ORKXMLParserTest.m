...
@interface ORKTestTabData : NSObject {
    NSString *_title;
    NSString *_summary;
}
...
@interface ORKXMLParserTest : ORKTestCase {

}

@end

@implementation ORKXMLParserTest
...
- (void)testShouldMapASingleXMLObjectPayloadToADictionary
{
    ...
    ORKXMLParserXMLReader *parser = [[ORKXMLParserXMLReader new] autorelease];
...
{
    ORKObjectMapping *mapping = [ORKObjectMapping mappingForClass:[ORKTestTabData class]];
    [mapping mapAttributes:@"title", @"summary", nil];
    ORKObjectMappingProvider *provider = [[ORKObjectMappingProvider alloc] init];
    id data = [ORKTestFixture parsedObjectWithContentsOfFixture:@"tab_data.xml"];
    ...
    ORKObjectMapper *mapper = [ORKObjectMapper mapperWithObject:data mappingProvider:provider];
...
{
    NSString *XML = [ORKTestFixture stringWithContentsOfFixture:@"container_attributes.xml"];
    NSError *error = [[NSError alloc] init];
    ORKXMLParserXMLReader *parser = [[ORKXMLParserXMLReader new] autorelease];
...
{
    NSString *XML = [ORKTestFixture stringWithContentsOfFixture:@"attributes_without_text_content.xml"];
    NSError *error = [[NSError alloc] init];
    ORKXMLParserXMLReader *parser = [[ORKXMLParserXMLReader new] autorelease];
    NSDictionary *result = [parser objectFromString:XML error:&error];
    ...
    assertThat([secondCurrency objectForKey:@"name"], is(equalTo(@"HORK")));
...
{
    NSString *XML = [ORKTestFixture stringWithContentsOfFixture:@"orders.xml"];
    NSError *error = [[NSError alloc] init];
    ORKXMLParserXMLReader *parser = [[ORKXMLParserXMLReader new] autorelease];
...
{
    NSString *XML = [ORKTestFixture stringWithContentsOfFixture:@"zend.xml"];
    NSError *error = [[NSError alloc] init];
    ORKXMLParserXMLReader *parser = [[ORKXMLParserXMLReader new] autorelease];
...
{
    NSString *XML = [ORKTestFixture stringWithContentsOfFixture:@"national_weather_service.xml"];
    NSError *error = [[NSError alloc] init];
    ORKXMLParserXMLReader *parser = [[ORKXMLParserXMLReader new] autorelease];
    NSException *exception = nil;
    @try {
...
- (void)testShouldParseXMLElementsAndAttributesProperly
{
    ...
    NSString *XML = [ORKTestFixture stringWithContentsOfFixture:@"channels.xml"];
    NSError *error = [[NSError alloc] init];
    ORKXMLParserXMLReader *parser = [[ORKXMLParserXMLReader new] autorelease];
    NSDictionary *result = [parser objectFromString:XML error:&error];
