//
//  ORKObjectMappingProviderTest.m
//  RestKit
//
...
@interface ORKObjectMappingProviderTest : ORKTestCase {
    ORKObjectManager *_objectManager;
}

@end

@implementation ORKObjectMappingProviderTest
...
- (void)testShouldFindAnExistingObjectMappingForAClass
{
    ...
    ORKObjectMappingDefinition *returnedMapping = [_objectManager.mappingProvider objectMappingForClass:[ORKHuman class]];
...
- (void)testShouldFindAnExistingObjectMappingForAKeyPath
{
    ...
    ORKObjectMappingDefinition *returnedMapping = [_objectManager.mappingProvider mappingForKeyPath:@"cat"];
    assertThat(returnedMapping, isNot(equalTo(nil)));
...
- (void)testShouldAllowYouToRemoveAMappingByKeyPath
{
    ...
    ORKObjectMappingDefinition *returnedMapping = [mappingProvider mappingForKeyPath:@"cat"];
    assertThat(returnedMapping, isNot(equalTo(nil)));
...
{
    ORKObjectMappingProvider *mappingProvider = [ORKObjectMappingProvider mappingProvider];
    ORKObjectMapping *mapping = [ORKObjectMapping mappingForClass:[NSMutableArray class]];
...
{
    ORKObjectMappingProvider *mappingProvider = [ORKObjectMappingProvider mappingProvider];
    ORKObjectMapping *mapping_1 = [ORKObjectMapping mappingForClass:[NSMutableArray class]];
    [mappingProvider addMapping:mapping_1 context:1];
    ORKObjectMapping *mapping_2 = [ORKObjectMapping mappingForClass:[NSMutableArray class]];
...
- (void)testSettingMappingForPathMatcherCreatesOrderedDictionary
{
    ...
    assertThat(contextValue, is(instanceOf([ORKOrderedDictionary class])));
}
...
- (void)testSettingMappingForPathMatcherCreatesDictionaryWithMappingAsValue
{
    ORKObjectMappingProvider *mappingProvider = [ORKObjectMappingProvider mappingProvider];
    ORKObjectMapping *mapping = [ORKObjectMapping mappingForClass:[NSMutableArray class]];
    ORKObjectMappingProviderContextEntry *entry = [ORKObjectMappingProviderContextEntry contextEntryWithMapping:mapping];
    [mappingProvider setMapping:mapping forPattern:@"/articles/:id" context:1];
    NSDictionary *contextValue = [mappingProvider valueForContext:1];
...
- (void)testRetrievalOfMappingForPathMatcher
{
    ...
    ORKObjectMappingDefinition *matchedMapping = [mappingProvider mappingForPatternMatchingString:@"/articles/12345" context:1];
...
- (void)testRetrievalOfMappingForPathMatcherIncludingQueryParameters
{
    ...
    ORKObjectMappingDefinition *matchedMapping = [mappingProvider mappingForPatternMatchingString:@"/articles/12345?page=5&this=that" context:1];
    assertThat(matchedMapping, is(equalTo(mapping)));
}
...
- (void)testRetrievalOfMappingForPathMatcherWithMultipleEntries
{
    ...
    ORKObjectMapping *mapping_3 = [ORKObjectMapping mappingForClass:[NSMutableArray class]];
    [mappingProvider setMapping:mapping_3 forPattern:@"/articles/:id\\.xml" context:1];

    ORKObjectMapping *mapping_4 = [ORKObjectMapping mappingForClass:[NSMutableArray class]];
...
- (void)testRetrievalOfEntryForPathMatcher
{
    ...
    ORKObjectMappingProviderContextEntry *matchingEntry = [mappingProvider entryForPatternMatchingString:@"/articles/12345"
                                                                                                context:1];
...
- (void)testRetrievalOfEntryForPathMatcherIncludingQueryParameters
{
    ...
    ORKObjectMappingProviderContextEntry *matchingEntry = [mappingProvider entryForPatternMatchingString:@"/articles/12345?page=5&this=that"
                                                                                                context:1];
    assertThat(matchingEntry, is(equalTo(entry)));
}
