//
//  ORKObjectMappingOperationTest.m
//  RestKit
//
...
#import "ORKMappableAssociation.h"
...
- (BOOL)validateBoolString:(id *)ioValue error:(NSError **)outError
{
    if ([(NSObject *)*ioValue isKindOfClass:[NSString class]] && [(NSString *)*ioValue isEqualToString:@"FAIL"]) {
        *outError = [NSError errorWithDomain:ORKErrorDomain code:ORKObjectMapperErrorUnmappableContent userInfo:nil];
        return NO;
...
@interface ORKObjectMappingOperationTest : ORKTestCase {

}

@end

@implementation ORKObjectMappingOperationTest

- (void)testShouldNotUpdateEqualURLProperties
{
    ORKObjectMapping *mapping = [ORKObjectMapping mappingForClass:[TestMappable class]];
    ...
    ORKObjectMappingOperation *operation = [[ORKObjectMappingOperation alloc] initWithSourceObject:dictionary destinationObject:object mapping:mapping];
...
- (void)testShouldSuccessfullyMapBoolsToStrings
{
    ...
    id<ORKParser> parser = [[ORKParserRegistry sharedRegistry] parserForMIMEType:@"application/json"];
    id data = [parser objectFromString:@"{\"boolString\":true}" error:nil];

    ORKObjectMappingOperation *operation = [[ORKObjectMappingOperation alloc] initWithSourceObject:data destinationObject:object mapping:mapping];
    BOOL success = [operation performMapping:nil];
    assertThatBool(success, is(equalToBool(YES)));
...
- (void)testShouldMapADateToAStringUsingThePreferredDateFormatter
{
    ...
    ORKObjectMappingOperation *operation = [[ORKObjectMappingOperation alloc] initWithSourceObject:object destinationObject:newObject mapping:mapping];
    NSError *error = nil;
    BOOL success = [operation performMapping:&error];
    assertThatBool(success, is(equalToBool(YES)));
...
- (void)testShouldGenerateAnUnknownKeyPathExceptionWhenIgnoreUnknownKeyPathsIsNO
{
    ...
    ORKObjectMappingOperation *operation = [[ORKObjectMappingOperation alloc] initWithSourceObject:object destinationObject:dictionary mapping:mapping];
    NSError *error = nil;
    BOOL success;
...
- (void)testShouldOptionallyIgnoreUnknownKeyPathRelationships
{
    ORKObjectMapping *mapping = [ORKObjectMapping mappingForClass:[NSMutableDictionary class]];
    [mapping mapAttributes:@"boolString", nil];
    [mapping mapRelationship:@"invalid" withMapping:[ORKObjectMapping mappingForClass:[TestMappable class]]];
    mapping.ignoreUnknownKeyPaths = YES;
    TestMappable *object = [[[TestMappable alloc] init] autorelease];
    object.boolString = @"test";
    NSMutableDictionary *dictionary = [NSMutableDictionary dictionary];
...
- (void)testShouldLogADebugMessageIfTheRelationshipMappingTargetsAnArrayOfArrays
{
    ...
    id data = [ORKTestFixture parsedObjectWithContentsOfFixture:@"ArrayOfNestedDictionaries.json"];
    ORKObjectMapping *objectMapping = [ORKObjectMapping mappingForClass:[ORKMappableObject class]];
    [objectMapping mapKeyPath:@"name" toAttribute:@"stringTest"];
    ORKObjectMapping *relationshipMapping = [ORKObjectMapping mappingForClass:[ORKMappableAssociation class]];
    ...
    ORKMappableObject *targetObject = [[ORKMappableObject new] autorelease];
    ORKLogToComponentWithLevelWhileExecutingBlock(lcl_cRestKitObjectMapping, ORKLogLevelDebug, ^ {
        ORKObjectMappingOperation *operation = [[ORKObjectMappingOperation alloc] initWithSourceObject:data
