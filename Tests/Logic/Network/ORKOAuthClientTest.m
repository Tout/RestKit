...
#import "ORKPortCheck.h"

#define ORKOAuthClientTestSkipWithoutMongoDB() \
    if (! [self isMongoRunning]) { \
        NSLog(@"!! Skipping OAuth Test: MongoDB not running"); \
        return; \
    }

@interface ORKOAuthClientTest : ORKTestCase

@end

@implementation ORKOAuthClientTest

- (BOOL)isMongoRunning
{
    static ORKPortCheck *portCheck = nil;
    if (! portCheck) {
        portCheck = [[ORKPortCheck alloc] initWithHost:@"localhost" port:27017];
        [portCheck run];
    }
...
{
    ORKOAuthClientTestSkipWithoutMongoDB();

    ORKTestResponseLoader *loader = [ORKTestResponseLoader responseLoader];
    ORKOAuthClient *client = ORKTestNewOAuthClient(loader);
...
- (void)testShouldGetProtectedResource
{
    ...
    ORKTestResponseLoader *resourceLoader = [ORKTestResponseLoader responseLoader];
    ORKClient *requestClient = [ORKClient clientWithBaseURLString:[client authorizationURL]];
    requestClient.OAuth2AccessToken = client.accessToken;
    requestClient.authenticationType = ORKRequestAuthenticationTypeOAuth2;
    ORKRequest *request = [requestClient requestWithResourcePath:@"/oauth2/pregen/me"];
