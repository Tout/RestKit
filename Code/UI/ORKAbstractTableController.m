...
#import "ORKRefreshGestureRecognizer.h"
...
NSString * const ORKTableControllerDidStartLoadNotification = @"ORKTableControllerDidStartLoadNotification";
NSString * const ORKTableControllerDidFinishLoadNotification = @"ORKTableControllerDidFinishLoadNotification";
NSString * const ORKTableControllerDidLoadObjectsNotification = @"ORKTableControllerDidLoadObjectsNotification";
NSString * const ORKTableControllerDidLoadEmptyNotification = @"ORKTableControllerDidLoadEmptyNotification";
NSString * const ORKTableControllerDidLoadErrorNotification = @"ORKTableControllerDidLoadErrorNotification";
NSString * const ORKTableControllerDidBecomeOnline = @"ORKTableControllerDidBecomeOnline";
NSString * const ORKTableControllerDidBecomeOffline = @"ORKTableControllerDidBecomeOffline";

static NSString *lastUpdatedDateDictionaryKey = @"lastUpdatedDateDictionaryKey";

@implementation ORKAbstractTableController
...
- (id)init
{
    self = [super init];
    if (self) {
        if ([self isMemberOfClass:[ORKAbstractTableController class]]) {
        ...
        self.state = ORKTableControllerStateNotYetLoaded;
        self.objectManager = [ORKObjectManager sharedManager];
        _cellMappings = [ORKTableViewCellMappings new];
...
- (void)setObjectManager:(ORKObjectManager *)objectManager
{
    ...
    if (_objectManager) {
        [notificationCenter removeObserver:self
                                      name:ORKObjectManagerDidBecomeOfflineNotification
                                    object:_objectManager];
        [notificationCenter removeObserver:self
                                      name:ORKObjectManagerDidBecomeOnlineNotification
                                    object:_objectManager];
    }
    ...
    if (objectManager) {
        ...
        if (objectManager.networkStatus != ORKObjectManagerNetworkStatusUnknown) {
            if (objectManager.isOnline) {
                self.state &= ~ORKTableControllerStateOffline;
            } else {
                self.state |= ORKTableControllerStateOffline;
            }
        }
    ...
    if (_autoRefreshFromNetwork != autoRefreshFromNetwork) {
        ...
        if (_autoRefreshFromNetwork) {
            NSString *cachePath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0]
                                   stringByAppendingPathComponent:@"ORKAbstractTableControllerCache"];
            _cache = [[ORKCache alloc] initWithPath:cachePath subDirectories:nil];
        } else {
            if (_cache) {
...
{
    if (loading) {
        self.state |= ORKTableControllerStateLoading;
    } else {
        self.state &= ~ORKTableControllerStateLoading;
    }
}
...
{
    if (empty) {
        self.state |= ORKTableControllerStateEmpty;
    } else {
        self.state &= ~ORKTableControllerStateEmpty;
    }
}
...
{
    if (error) {
        self.state |= ORKTableControllerStateError;
    } else {
        self.state &= ~ORKTableControllerStateError;
    }
}
...
{
    ORKLogTrace(@"%@ received network status change notification: %@", self, [notification name]);
...
- (UITableViewCell *)tableView:(UITableView *)theTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ORKLogTrace(@"%@ cellForRowAtIndexPath:%@ = %@", self, indexPath, cell);
    return cell;
}
...
- (void)tableView:(UITableView *)theTableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ORKLogTrace(@"%@: Row at indexPath %@ selected for tableView %@", self, indexPath, theTableView);

    id object = [self objectForRowAtIndexPath:indexPath];

    // NOTE: Do NOT use cellForObjectAtIndexPath here. See https://gist.github.com/eafbb641d37bb7137759
    UITableViewCell *cell = [theTableView cellForRowAtIndexPath:indexPath];
    ORKTableViewCellMapping *cellMapping = [_cellMappings cellMappingForObject:object];
    ...
    if (cellMapping.onSelectCellForObjectAtIndexPath) {
        ORKLogTrace(@"%@: Invoking onSelectCellForObjectAtIndexPath block with cellMapping %@ for object %@ at indexPath = %@", self, cell, object, indexPath);
...
- (void)tableView:(UITableView *)theTableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ORKTableViewCellMapping *cellMapping = [self.cellMappings cellMappingForObject:mappableObject];
...
- (CGFloat)tableView:(UITableView *)theTableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.variableHeightRows) {
        ORKTableViewCellMapping *cellMapping = [self cellMappingForObjectAtIndexPath:indexPath];
        ...
        if (cellMapping.heightOfCellForObjectAtIndexPath) {
            ...
            ORKLogTrace(@"Variable row height configured for tableView. Height via block invocation for row at indexPath '%@' = %f", indexPath, cellMapping.rowHeight);
            return height;
        } else {
            ORKLogTrace(@"Variable row height configured for tableView. Height for row at indexPath '%@' = %f", indexPath, cellMapping.rowHeight);
            return cellMapping.rowHeight;
        }
    }

    ORKLogTrace(@"Uniform row height configured for tableView. Table view row height = %f", self.tableView.rowHeight);
...
- (void)tableView:(UITableView *)theTableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cellMapping.onTapAccessoryButtonForObjectAtIndexPath) {
        ORKLogTrace(@"Found a block for tableView:accessoryButtonTappedForRowWithIndexPath: Executing...");
...
- (NSString *)tableView:(UITableView *)theTableView titleForDeleteConfirmationButtonForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cellMapping.titleForDeleteButtonForObjectAtIndexPath) {
        ORKLogTrace(@"Found a block for tableView:titleForDeleteConfirmationButtonForRowAtIndexPath: Executing...");
        UITableViewCell *cell = [self tableView:self.tableView cellForRowAtIndexPath:indexPath];
        id object = [self objectForRowAtIndexPath:indexPath];
...
- (UITableViewCellEditingStyle)tableView:(UITableView *)theTableView editingStyleForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_canEditRows) {
        ...
        if (cellMapping.editingStyleForObjectAtIndexPath) {
            ORKLogTrace(@"Found a block for tableView:editingStyleForRowAtIndexPath: Executing...");
            id object = [self objectForRowAtIndexPath:indexPath];
...
{
    if (_canMoveRows) {
        ORKTableViewCellMapping *cellMapping = [self cellMappingForObjectAtIndexPath:sourceIndexPath];
        if (cellMapping.targetIndexPathForMove) {
            ORKLogTrace(@"Found a block for tableView:targetIndexPathForMoveFromRowAtIndexPath:toProposedIndexPath: Executing...");
...
- (NSDate *)lastUpdatedDate
{
    ...
    if (_autoRefreshFromNetwork) {
        ...
        ORKLogTrace(@"Last updated dates dictionary retrieved from tableController cache: %@", lastUpdatedDates);
        if (lastUpdatedDates) {
...
- (BOOL)isAutoRefreshNeeded
{
    ...
    if (_autoRefreshFromNetwork) {
        ...
        ORKLogTrace(@"Last updated: %@", lastUpdatedDate);
        if (lastUpdatedDate) {
            ORKLogTrace(@"-timeIntervalSinceNow=%f, autoRefreshRate=%f",
...
#pragma mark - ORKRequestDelegate & ORKObjectLoaderDelegate methods

- (void)requestDidStartLoad:(ORKRequest *)request
{
    ORKLogTrace(@"tableController %@ started loading.", self);
    [self didStartLoad];
}

- (void)requestDidCancelLoad:(ORKRequest *)request
{
    ORKLogTrace(@"tableController %@ cancelled loading.", self);
    self.loading = NO;
...
{
    ORKLogTrace(@"tableController %@ timed out while loading.", self);
    self.loading = NO;
}

- (void)request:(ORKRequest *)request didLoadResponse:(ORKResponse *)response
{
    ORKLogTrace(@"tableController %@ finished loading.", self);
    ...
    if (self.autoRefreshFromNetwork) {
        ...
        ORKLogTrace(@"Setting timeIntervalSince1970=%@ for URL %@", timeIntervalSince1970, [request.URL absoluteString]);
...
{
    ORKLogError(@"tableController %@ failed network load with error: %@", self, error);
...
- (void)didFinishLoad
{
    ...
    if (![self isEmpty] && ![self isLoading]) {
        [[NSNotificationCenter defaultCenter] postNotificationName:ORKTableControllerDidLoadObjectsNotification object:self];
    }
...
{
    switch (state) {
        case ORKTableControllerStateNormal:
        case ORKTableControllerStateLoading:
        case ORKTableControllerStateNotYetLoaded:
            break;

        case ORKTableControllerStateEmpty:
            return self.imageForEmpty;
            break;

        case ORKTableControllerStateError:
            return self.imageForError;
            break;

        case ORKTableControllerStateOffline:
...
- (BOOL)isLoading
{
    return (self.state & ORKTableControllerStateLoading) != 0;
}

- (BOOL)isLoaded
{
    return (self.state & ORKTableControllerStateNotYetLoaded) == 0;
//    return self.state != ORKTableControllerStateNotYetLoaded;
}

- (BOOL)isOffline
{
    return (self.state & ORKTableControllerStateOffline) != 0;
}
- (BOOL)isOnline
{
...
- (BOOL)isError
{
    return (self.state & ORKTableControllerStateError) != 0;
}

- (BOOL)isEmpty
{
    return (self.state & ORKTableControllerStateEmpty) != 0;
}
...
- (void)isLoadingDidChange
{
    if ([self isLoading]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:ORKTableControllerDidStartLoadNotification object:self];
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:ORKTableControllerDidFinishLoadNotification object:self];
...
{
    if ([self isLoaded]) {
        ORKLogDebug(@"%@: is now loaded.", self);
    } else {
        ORKLogDebug(@"%@: is NOT loaded.", self);
    }
}
...
- (void)isErrorDidChange
{
    if ([self isError]) {
        ...
        NSDictionary *userInfo = [NSDictionary dictionaryWithObject:self.error forKey:ORKErrorNotificationErrorKey];
        [[NSNotificationCenter defaultCenter] postNotificationName:ORKTableControllerDidLoadErrorNotification object:self userInfo:userInfo];
    }
}
...
- (void)isEmptyDidChange
{
    if ([self isEmpty]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:ORKTableControllerDidLoadEmptyNotification object:self];
    }
}
...
- (void)isOnlineDidChange
{
    if ([self isOnline]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:ORKTableControllerDidBecomeOnline object:self];
    } else {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:ORKTableControllerDidBecomeOffline object:self];
    }
}

- (void)updateTableViewForStateChange:(NSDictionary *)change
{
    ORKTableControllerState oldState = [[change valueForKey:NSKeyValueChangeOldKey] integerValue];
    ORKTableControllerState newState = [[change valueForKey:NSKeyValueChangeNewKey] integerValue];

    // Determine state transitions
    BOOL loadedChanged = ((oldState ^ newState) & ORKTableControllerStateNotYetLoaded);
    BOOL emptyChanged = ((oldState ^ newState) & ORKTableControllerStateEmpty);
    BOOL offlineChanged = ((oldState ^ newState) & ORKTableControllerStateOffline);
    BOOL loadingChanged = ((oldState ^ newState) & ORKTableControllerStateLoading);
    BOOL errorChanged = ((oldState ^ newState) & ORKTableControllerStateError);
    ...
    if (self.state == ORKTableControllerStateNormal) {
...
- (ORKRefreshGestureRecognizer *)pullToRefreshGestureRecognizer
{
    ORKRefreshGestureRecognizer *refreshRecognizer = nil;
    for (ORKRefreshGestureRecognizer *recognizer in self.tableView.gestureRecognizers) {
        if ([recognizer isKindOfClass:[ORKRefreshGestureRecognizer class]]) {
...
{
    ORKRefreshGestureRecognizer *recognizer = nil;
    if (pullToRefreshEnabled) {
        recognizer = [[[ORKRefreshGestureRecognizer alloc] initWithTarget:self action:@selector(pullToRefreshStateChanged:)] autorelease];
...
- (void)pullToRefreshStateChanged:(UIGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateRecognized) {
        if ([self pullToRefreshDataSourceIsLoading:gesture])
            return;
        ORKLogDebug(@"%@: pull to refresh triggered from gesture: %@", self, gesture);
        if (self.objectLoader) {
...
{
    ORKRefreshGestureRecognizer *recognizer = [self pullToRefreshGestureRecognizer];
    if (recognizer)
        [recognizer setRefreshState:ORKRefreshIdle];
}
...
- (void)removeSwipeView:(BOOL)animated
{
    if (!_cellSwipeViewsEnabled || !_swipeCell || _animatingCellSwipe) {
        ORKLogTrace(@"Exiting early with _cellSwipeViewsEnabled=%d, _swipCell=%@, _animatingCellSwipe=%d",
...
- (void)resizeTableViewForKeyboard:(NSNotification *)notification
{
    ...
    ORKLogTrace(@"keyboardEndFrame.size.height=%f, heightForViewShift=%f",
    ...
    if (navigationController && navigationController.toolbar && !navigationController.toolbarHidden) {
        ...
        ORKLogTrace(@"Found a visible toolbar. Reducing size of heightForViewShift by=%f", bottomBarOffset);
    }
    ...
    if (tabBarController && tabBarController.tabBar && !self.viewController.hidesBottomBarWhenPushed) {
        ...
        ORKLogTrace(@"Found a visible tabBar. Reducing size of heightForViewShift by=%f", bottomBarOffset);
    }
    ...
    if ([[notification name] isEqualToString:UIKeyboardWillShowNotification]) {
        ...
        ORKLogTrace(@"Searching for a firstResponder not inside our nonKeyboardRect (%f, %f, %f, %f)",
        ...
        if (firstResponder) {
            ...
            ORKLogTrace(@"Found firstResponder=%@ at (%f, %f, %f, %f)", firstResponder,
            ...
            if (![firstResponder.superview isEqual:self.tableView]) {
                ...
                ORKLogTrace(@"firstResponder (%@) frame is not in tableView's coordinate system. Coverted to (%f, %f, %f, %f)",
            ...
            if (!CGRectContainsPoint(nonKeyboardRect, firstResponderFrame.origin)) {
                ORKLogTrace(@"firstResponder (%@) is underneath keyboard. Beginning scroll of tableView to show", firstResponder);
...
- (void)loadTableWithObjectLoader:(ORKObjectLoader *)theObjectLoader
{
    ...
    if (! [self.objectLoader isEqual:theObjectLoader]) {
        if (self.objectLoader) {
            ORKLogDebug(@"Cancelling in progress table load: asked to load with a new object loader.");
