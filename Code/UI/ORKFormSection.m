...
@implementation ORKFormSection

@synthesize form = _form;

+ (id)sectionInForm:(ORKForm *)form
{
    return [[[self alloc] initWithForm:form] autorelease];
}

- (id)initWithForm:(ORKForm *)form
{
    self = [super init];
    if (self) {
        self.form = form;
...
- (UIControl *)controlWithType:(ORKFormControlType)controlType
{
    UIControl *control = nil;
    switch (controlType) {
        case ORKFormControlTypeTextField:
        case ORKFormControlTypeTextFieldSecure:;
            UITextField *textField = [[[UITextField alloc] init] autorelease];
            textField.secureTextEntry = (controlType == ORKFormControlTypeTextFieldSecure);
            control = (UIControl *)textField;
            break;

        case ORKFormControlTypeSwitch:;
            control = [(UIControl *)[UISwitch new] autorelease];
            break;

        case ORKFormControlTypeSlider:;
            control = [(UIControl *)[UISlider new] autorelease];
            break;

        case ORKFormControlTypeLabel:;
            control = [(UIControl *)[UILabel new] autorelease];
            break;

        case ORKFormControlTypeUnknown:
        default:
            break;
    }
...
- (void)addAttributeMapping:(ORKObjectAttributeMapping *)attributeMapping forKeyPath:(NSString *)attributeKeyPath toTableItem:(ORKTableItem *)tableItem
{
...
{
    ORKControlTableItem *tableItem = [ORKControlTableItem tableItemWithControl:control];
    ORKObjectAttributeMapping *attributeMapping = [[ORKObjectAttributeMapping new] autorelease];
