...
NSString * const ORKRefreshGestureAnimationKey       = @"ORKRefreshGestureAnimationKey";
NSString * const ORKRefreshResetGestureAnimationKey  = @"ORKRefreshResetGestureAnimationKey";
static CGFloat const kFlipArrowAnimationTime = 0.18f;
static CGFloat const kDefaultTriggerViewHeight = 64.f;

@interface ORKRefreshGestureRecognizer ()

- (CABasicAnimation *)triggeredAnimation;
- (CABasicAnimation *)idlingAnimation;

@property (nonatomic, retain, readwrite) ORKRefreshTriggerView *triggerView;
@property (nonatomic, assign) BOOL isBoundToScrollView;
@property (nonatomic, retain) NSDateFormatter *dateFormatter;

@end

@implementation ORKRefreshGestureRecognizer
...
- (id)initWithTarget:(id)target action:(SEL)action
{
    ...
    if (self) {
        _triggerView = [[ORKRefreshTriggerView alloc] initWithFrame:CGRectZero];
...
- (void)setRefreshState:(ORKRefreshState)refreshState
{
    ...
    switch (refreshState) {
        ...
        case ORKRefreshTriggered: {
            if (![self.triggerView.arrowView.layer animationForKey:ORKRefreshGestureAnimationKey])
                [self.triggerView.arrowView.layer addAnimation:[self triggeredAnimation] forKey:ORKRefreshGestureAnimationKey];
            self.triggerView.titleLabel.text = NSLocalizedString(@"Release to refresh...", @"Release to refresh status");
        }
            break;

        case ORKRefreshIdle: {
            if (_refreshState == ORKRefreshLoading) {
            ...
            } else if (_refreshState == ORKRefreshTriggered) {
                if ([self.triggerView.arrowView.layer animationForKey:ORKRefreshGestureAnimationKey]) {
                    [self.triggerView.arrowView.layer addAnimation:[self idlingAnimation] forKey:ORKRefreshResetGestureAnimationKey];
                }
            }
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (self.scrollView.contentOffset.y < -kDefaultTriggerViewHeight) {
        self.refreshState   = ORKRefreshTriggered;
        self.state          = UIGestureRecognizerStateChanged;
    } else if (self.state != UIGestureRecognizerStateRecognized) {
        self.refreshState   = ORKRefreshIdle;
        self.state          = UIGestureRecognizerStateChanged;
    }
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (self.refreshState == ORKRefreshTriggered) {
        self.refreshState = ORKRefreshLoading;
