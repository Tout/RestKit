...
+ (id)sectionUsingBlock:(void (^)(ORKTableSection *))block
{
    ORKTableSection *section = [self section];
    block(section);
    return section;
}

+ (id)sectionForObjects:(NSArray *)objects withMappings:(ORKTableViewCellMappings *)cellMappings
{
    return [self sectionUsingBlock:^(ORKTableSection *section) {
