...
@interface ORKTableController ()
@property (nonatomic, readwrite) NSMutableArray *sections;
@end

@implementation ORKTableController
...
- (void)removeAllSections:(BOOL)recreateFirstSection
{
    ...
    if (recreateFirstSection) {
        [self addSection:[ORKTableSection section]];
    }
}
...
- (void)loadObjects:(NSArray *)objects inSection:(NSUInteger)sectionIndex
{
    ...
    ORKTableSection *section = [self sectionAtIndex:sectionIndex];
...
{
    for (ORKTableItem *tableItem in tableItems) {
...
- (void)loadTableFromResourcePath:(NSString *)resourcePath usingBlock:(void (^)(ORKObjectLoader *loader))block
{
    NSAssert(self.objectManager, @"Cannot perform a network load without an object manager");
    ORKObjectLoader *objectLoader = [self.objectManager loaderWithResourcePath:resourcePath];
...
- (void)loadForm:(ORKForm *)form
{
    ...
    for (ORKFormSection *section in form.sections) {
...
- (void)tableView:(UITableView *)theTableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.canEditRows) {
        if (editingStyle == UITableViewCellEditingStyleDelete) {
            ORKTableSection *section = [self.sections objectAtIndex:indexPath.section];
...
- (void)objectLoader:(ORKObjectLoader *)loader didLoadObjects:(NSArray *)objects
{
...
{
    ORKTableSection *section = [self sectionAtIndex:indexPath.section];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)theTableView
{
    ...
    ORKLogTrace(@"%@ numberOfSectionsInTableView = %d", self, self.sectionCount);
...
- (NSIndexPath *)indexPathForObject:(id)object
{
    ...
    for (ORKTableSection *section in self.sections) {
...
- (BOOL)isConsideredEmpty
{
    ...
    ORKLogTrace(@"Determined isConsideredEmpty = %@. self.rowCount = %d with %d nonRowItems in the table", isEmpty ? @"YES" : @"NO", self.rowCount, nonRowItemsCount);
    return isEmpty;
}
