 ...
 */
typedef void(^ORKControlBlockActionBlock)(id sender);
@interface ORKControlBlockAction : NSObject {
@private
    ORKControlBlockActionBlock _actionBlock;
}
...
@implementation ORKControlBlockAction
...
@implementation ORKTableViewCellMapping
...
{
    ORKTableViewCellMapping *cellMapping = [self cellMapping];
...
+ (id)cellMappingUsingBlock:(void (^)(ORKTableViewCellMapping *))block
{
    ORKTableViewCellMapping *cellMapping = [self cellMapping];
    block(cellMapping);
    return cellMapping;
...
{
    ORKTableViewCellMapping *copy = [super copyWithZone:zone];
...
- (id)mappableObjectForData:(UITableView *)tableView
{
    ...
    ORKLogTrace(@"About to dequeue reusable cell using self.reuseIdentifier=%@", self.reuseIdentifier);
...
{
    [self addPrepareCellBlock:^(UITableViewCell *cell) {
        ORKControlBlockAction *blockAction = [ORKControlBlockAction actionWithBlock:block];
        UIControl *control = [cell valueForKeyPath:keyPath];
        if (control) {
