//
//  ORKFetchedResultsTableController.m
//  RestKit
//
...
@interface ORKFetchedResultsTableController ()
...
@implementation ORKFetchedResultsTableController
...
- (BOOL)performFetch:(NSError **)error
{
    ...
    if (!success) {
        ORKLogError(@"performFetch failed with error: %@", [*error localizedDescription]);
        return NO;
    } else {
        ORKLogTrace(@"performFetch completed successfully");
...
- (void)loadTable
{
    ...
    NSAssert(fetchRequest != nil, @"Attempted to load ORKFetchedResultsTableController with nil fetchRequest for resourcePath %@, fetchRequest %@", _resourcePath, _fetchRequest);
...
- (UITableViewCell *)cellForObjectAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ORKObjectMappingOperation *mappingOperation = [[ORKObjectMappingOperation alloc] initWithSourceObject:mappableObject destinationObject:cell mapping:cellMapping];
    NSError *error = nil;
    ...
    if (success == NO && error != nil) {
        ORKLogError(@"Failed to generate table cell for object: %@", error);
        return nil;
    }
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)theTableView
{
    ...
    ORKLogTrace(@"numberOfSectionsInTableView: %d (%@)", [[_fetchedResultsController sections] count], [[_fetchedResultsController sections] valueForKey:@"name"]);
    return [[_fetchedResultsController sections] count];
}
...
- (NSInteger)tableView:(UITableView *)theTableView numberOfRowsInSection:(NSInteger)section
{
    ...
    ORKLogTrace(@"%@ numberOfRowsInSection:%d = %d", self, section, self.sectionCount);
...
- (void)tableView:(UITableView *)theTableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.canEditRows && editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        ORKObjectMapping *mapping = [[ORKObjectManager sharedManager].mappingProvider objectMappingForClass:[managedObject class]];
        if ([mapping isKindOfClass:[ORKManagedObjectMapping class]]) {
            ORKManagedObjectMapping *managedObjectMapping = (ORKManagedObjectMapping *)mapping;
            ...
            if ([managedObject valueForKeyPath:primaryKeyAttribute]) {
                ORKLogTrace(@"About to fire a delete request for managedObject: %@", managedObject);
                [[ORKObjectManager sharedManager] deleteObject:managedObject delegate:self];
            } else {
                ORKLogTrace(@"About to locally delete managedObject: %@", managedObject);
                [managedObject.managedObjectContext deleteObject:managedObject];

                NSError *error = nil;
                [managedObject.managedObjectContext save:&error];
                if (error) {
                    ORKLogError(@"Failed to save managedObjectContext after a delete with error: %@", error);
                }
            }
        }
    }
...
- (void)loadTableFromNetwork
{
    ...
    ORKLogTrace(@"About to loadTableWithObjectLoader...");
...
- (BOOL)isConsideredEmpty
{
    ...
    ORKLogTrace(@"Determined isEmpty = %@. fetchedObjects count = %d", isEmpty ? @"YES" : @"NO", fetchedObjectsCount);
    return isEmpty;
}
...
{
    ORKLogTrace(@"Beginning updates for fetchedResultsController (%@). Current section count = %d (resource path: %@)", controller, [[controller sections] count], _resourcePath);
     ...
     forChangeType:(NSFetchedResultsChangeType)type
{
    ...
    switch (type) {
        ...
        default:
            ORKLogTrace(@"Encountered unexpected section changeType: %d", type);
            break;
    }
}
      ...
      newIndexPath:(NSIndexPath *)newIndexPath
{
    ...
    switch (type) {
        ...
        default:
            ORKLogTrace(@"Encountered unexpected object changeType: %d", type);
            break;
    }
}
...
{
    ORKLogTrace(@"Ending updates for fetchedResultsController (%@). New section count = %d (resource path: %@)",
