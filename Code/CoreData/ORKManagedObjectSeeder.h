...
@protocol ORKManagedObjectSeederDelegate
@required
 ...
 * and provided to ORKManagedObjectStore at initialization to start the app with a set of
 * data immediately available for use within Core Data.
 */
@interface ORKManagedObjectSeeder : NSObject {
    ORKObjectManager *_manager;
}

// Delegate for seeding operations
@property (nonatomic, assign) NSObject<ORKManagedObjectSeederDelegate> *delegate;
 ...
 */
+ (void)generateSeedDatabaseWithObjectManager:(ORKObjectManager *)objectManager fromFiles:(NSString *)fileName, ...;
