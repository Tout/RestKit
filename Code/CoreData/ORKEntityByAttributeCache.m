...
#define ORKLogComponent lcl_cRestKitCoreDataCache

@interface ORKEntityByAttributeCache ()
@property (nonatomic, retain) NSMutableDictionary *attributeValuesToObjectIDs;
@end

@implementation ORKEntityByAttributeCache
...
- (BOOL)shouldCoerceAttributeToString:(NSString *)attributeValue
{
    ...
    Class attributeType = [[ORKObjectPropertyInspector sharedInspector] typeForProperty:self.attribute ofEntity:self.entity];
    return [attributeType instancesRespondToSelector:@selector(stringValue)];
}

- (void)load
{
    ORKLogDebug(@"Loading entity cache for Entity '%@' by attribute '%@'", self.entity.name, self.attribute);
    ...
    if (error) {
        ORKLogError(@"Failed to load entity cache: %@", error);
        return;
    }
    ...
    for (NSManagedObjectID *objectID in objectIDs) {
        ...
        if (! object && error) {
            ORKLogError(@"Failed to retrieve managed object with ID %@: %@", objectID, error);
        }
...
- (void)flush
{
    ORKLogDebug(@"Flushing entity cache for Entity '%@' by attribute '%@'", self.entity.name, self.attribute);
...
- (NSManagedObject *)objectWithID:(NSManagedObjectID *)objectID
{
    ...
    NSManagedObject *object = [self.managedObjectContext existingObjectWithID:objectID error:&error];
    if (! object && error) {
        ORKLogError(@"Failed to retrieve managed object with ID %@. Error %@\n%@", objectID, [error localizedDescription], [error userInfo]);
        return nil;
    }
...
- (void)addObject:(NSManagedObject *)object
{
    ...
    if (attributeValue) {
        ...
        ORKLogWarning(@"Unable to add object with nil value for attribute '%@': %@", self.attribute, object);
    }
}
...
- (void)removeObject:(NSManagedObject *)object
{
    ...
    if (attributeValue) {
        ...
        ORKLogWarning(@"Unable to remove object with nil value for attribute '%@': %@", self.attribute, object);
    }
}
...
- (void)managedObjectContextDidChange:(NSNotification *)notification
{
    ...
    ORKLogTrace(@"insertedObjects=%@, updatedObjects=%@, deletedObjects=%@", insertedObjects, updatedObjects, deletedObjects);
