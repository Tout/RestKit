//
//  ORKObjectPropertyInspector+CoreData.m
//  RestKit
//
...
ORK_FIX_CATEGORY_BUG(ORKObjectPropertyInspector_CoreData)

// Set Logging Component
#undef ORKLogComponent
#define ORKLogComponent lcl_cRestKitCoreData

@implementation ORKObjectPropertyInspector (CoreData)
...
- (NSDictionary *)propertyNamesAndTypesForEntity:(NSEntityDescription *)entity
{
    ...
    for (NSString *name in [entity attributesByName]) {
        ...
        if ([attributeDescription attributeValueClassName]) {
            ...
            const char *destinationClassName = [[ORKObjectPropertyInspector propertyTypeFromAttributeString:attributeString] cStringUsingEncoding:NSUTF8StringEncoding];
    ...
    ORKLogDebug(@"Cached property names and types for Entity '%@': %@", entity, propertyNamesAndTypes);
    return propertyNamesAndTypes;
}
