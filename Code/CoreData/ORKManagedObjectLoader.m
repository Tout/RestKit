...
@implementation ORKManagedObjectLoader

@synthesize objectStore = _objectStore;

+ (id)loaderWithURL:(ORKURL *)URL mappingProvider:(ORKObjectMappingProvider *)mappingProvider objectStore:(ORKManagedObjectStore *)objectStore
{
    return [[[self alloc] initWithURL:URL mappingProvider:mappingProvider objectStore:objectStore] autorelease];
}

- (id)initWithURL:(ORKURL *)URL mappingProvider:(ORKObjectMappingProvider *)mappingProvider objectStore:(ORKManagedObjectStore *)objectStore
{
...
#pragma mark - ORKObjectMapperDelegate methods

- (void)objectMapper:(ORKObjectMapper *)objectMapper didMapFromObject:(id)sourceObject toObject:(id)destinationObject atKeyPath:(NSString *)keyPath usingMapping:(ORKObjectMapping *)objectMapping
{
...
#pragma mark - ORKObjectLoader overrides
...
- (void)deleteCachedObjectsMissingFromResult:(ORKObjectMappingResult *)result
{
    if (! [self isGET]) {
        ORKLogDebug(@"Skipping cleanup of objects via managed object cache: only used for GET requests.");
        return;
    }
    ...
    if ([self.URL isKindOfClass:[ORKURL class]]) {
        ...
        for (id object in cachedObjects) {
            if (NO == [results containsObject:object]) {
                ORKLogTrace(@"Deleting orphaned object %@: not found in result set and expected at this resource path", object);
                [[self.objectStore managedObjectContextForCurrentThread] deleteObject:object];
            }
        }
    } else {
        ORKLogWarning(@"Unable to perform cleanup of server-side object deletions: unable to determine resource path.");
    }
}
...
- (void)processMappingResult:(ORKObjectMappingResult *)result
{
    ...
    if (_targetObjectID && self.targetObject && self.method == ORKRequestMethodDELETE) {
        NSManagedObject *backgroundThreadObject = [self.objectStore objectWithID:_targetObjectID];
        ORKLogInfo(@"Deleting local object %@ due to DELETE request", backgroundThreadObject);
    ...
    if ([self.response isSuccessful]) {
        ...
        if (! success) {
            ORKLogError(@"Failed to save managed object context after mapping completed: %@", [error localizedDescription]);
...
- (void)handleResponseError
{
    ...
    if (_targetObjectID) {
        if (_deleteObjectOnFailure) {
            ORKLogInfo(@"Error response encountered: Deleting existing managed object with ID: %@", _targetObjectID);
            ...
            if (objectToDelete) {
                ...
                ORKLogWarning(@"Unable to delete existing managed object with ID: %@. Object not found in the store.", _targetObjectID);
            }
        } else {
            ORKLogDebug(@"Skipping deletion of existing managed object");
        }
    }
}
...
- (BOOL)isResponseMappable
{
    if ([self.response wasLoadedFromCache]) {
        NSArray *cachedObjects = [self cachedObjects];
        if (! cachedObjects) {
            ORKLogDebug(@"Skipping managed object mapping optimization -> Managed object cache returned nil cachedObjects for resourcePath: %@", self.resourcePath);
