...
@implementation ORKManagedObjectMapping
...
+ (id)mappingForClass:(Class)objectClass inManagedObjectStore:(ORKManagedObjectStore *)objectStore
{
    return [self mappingForEntityWithName:NSStringFromClass(objectClass) inManagedObjectStore:objectStore];
}

+ (ORKManagedObjectMapping *)mappingForEntity:(NSEntityDescription *)entity inManagedObjectStore:(ORKManagedObjectStore *)objectStore
{
    return [[[self alloc] initWithEntity:entity inManagedObjectStore:objectStore] autorelease];
}

+ (ORKManagedObjectMapping *)mappingForEntityWithName:(NSString *)entityName inManagedObjectStore:(ORKManagedObjectStore *)objectStore
{
...
- (id)initWithEntity:(NSEntityDescription *)entity inManagedObjectStore:(ORKManagedObjectStore *)objectStore
{
    NSAssert(entity, @"Cannot initialize an ORKManagedObjectMapping without an entity. Maybe you want ORKObjectMapping instead?");
...
{
    NSAssert([_relationshipToPrimaryKeyMappings objectForKey:relationshipName] == nil, @"Cannot add connect relationship %@ by primary key, a mapping already exists.", relationshipName);
    ORKDynamicObjectMappingMatcher *matcher = [[ORKDynamicObjectMappingMatcher alloc] initWithKey:keyPath value:value primaryKeyAttribute:primaryKeyAttribute];
...
{
    NSAssert([_relationshipToPrimaryKeyMappings objectForKey:relationshipName] == nil, @"Cannot add connect relationship %@ by primary key, a mapping already exists.", relationshipName);
    ORKDynamicObjectMappingMatcher *matcher = [[ORKDynamicObjectMappingMatcher alloc] initWithPrimaryKeyAttribute:primaryKeyAttribute evaluationBlock:block];
    [_relationshipToPrimaryKeyMappings setObject:matcher forKey:relationshipName];
    [matcher release];
}
...
- (id)mappableObjectForData:(id)mappableData
{
    ...
    ORKObjectAttributeMapping *primaryKeyAttributeMapping = nil;
    ...
    if (primaryKeyAttribute) {
        ...
        if ([primaryKeyAttributeMapping isMappingForKeyOfNestedDictionary]) {
            ORKLogDebug(@"Detected use of nested dictionary key as primaryKey attribute...");
            ...
            if (keyPathForPrimaryKeyElement) {
                ...
                ORKLogWarning(@"Unable to find source attribute for primaryKeyAttribute '%@': unable to find existing object instances by primary key.", primaryKeyAttribute);
            }
        }
    }
...
- (Class)classForProperty:(NSString *)propertyName
{
    ...
    if (! propertyClass) {
        propertyClass = [[ORKObjectPropertyInspector sharedInspector] typeForProperty:propertyName ofEntity:self.entity];
    }
