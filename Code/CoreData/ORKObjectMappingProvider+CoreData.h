//
//  ORKObjectMappingProvider+CoreData.h
//  RestKit
//
...
typedef NSFetchRequest *(^ORKObjectMappingProviderFetchRequestBlock)(NSString *resourcePath);

/**
 Provides extensions to ORKObjectMappingProvider to support Core Data specific
 functionality.
 */
@interface ORKObjectMappingProvider (CoreData)
 ...
 **NOTE** that care must be taken when configuring patterns within the provider. The patterns
 ...
 @see ORKObjectLoader
 */
- (void)setObjectMapping:(ORKObjectMappingDefinition *)objectMapping forResourcePathPattern:(NSString *)resourcePathPattern withFetchRequestBlock:(ORKObjectMappingProviderFetchRequestBlock)fetchRequestBlock;
