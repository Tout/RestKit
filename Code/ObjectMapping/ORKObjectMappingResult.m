...
@implementation ORKObjectMappingResult
...
+ (ORKObjectMappingResult *)mappingResultWithDictionary:(NSDictionary *)keyPathToMappedObjects
{
...
- (id)asObject
{
    ...
    if (count > 1) ORKLogWarning(@"Coerced object mapping result containing %lu objects into singular result.", (unsigned long)count);
...
- (NSError *)asError
{
    ...
    if ([collection count] > 0) {
        ...
        ORKLogWarning(@"Expected mapping result to contain at least one object to construct an error");
    }
    NSDictionary *userInfo = [NSDictionary dictionaryWithObjectsAndKeys:collection, ORKObjectMapperErrorObjectsKey,
                              description, NSLocalizedDescriptionKey, nil];

    NSError *error = [NSError errorWithDomain:ORKErrorDomain code:ORKObjectMapperErrorFromMappingResult userInfo:userInfo];
    return error;
}
