...
@implementation ORKObjectSerializer

@synthesize object = _object;
@synthesize mapping = _mapping;

+ (id)serializerWithObject:(id)object mapping:(ORKObjectMapping *)mapping
{
    return [[[self alloc] initWithObject:object mapping:mapping] autorelease];
}

- (id)initWithObject:(id)object mapping:(ORKObjectMapping *)mapping
{
    self = [super init];
    if (self) {
...
- (id)serializedObject:(NSError **)error
{
    ...
    ORKObjectMappingOperation *operation = [ORKObjectMappingOperation mappingOperationFromObject:_object toObject:dictionary withMapping:_mapping];
...
- (id<ORKRequestSerializable>)serializationForMIMEType:(NSString *)MIMEType error:(NSError **)error
{
    if ([MIMEType isEqualToString:ORKMIMETypeFormURLEncoded]) {
        // Dictionaries are natively ORKRequestSerializable as Form Encoded
        return [self serializedObject:error];
    } else {
        NSString *string = [self serializedObjectForMIMEType:MIMEType error:error];
        if (string) {
            NSData *data = [string dataUsingEncoding:NSUTF8StringEncoding];
            return [ORKRequestSerialization serializationWithData:data MIMEType:MIMEType];
        }
    }
...
#pragma mark - ORKObjectMappingOperationDelegate
...
- (void)objectMappingOperation:(ORKObjectMappingOperation *)operation didSetValue:(id)value forKeyPath:(NSString *)keyPath usingMapping:(ORKObjectAttributeMapping *)mapping
{
    ...
    if (transformedValue) {
        ORKLogDebug(@"Serialized %@ value at keyPath to %@ (%@)", NSStringFromClass([value class]), NSStringFromClass([transformedValue class]), value);
