...
ORKParserRegistry *gSharedRegistry;

@implementation ORKParserRegistry

+ (ORKParserRegistry *)sharedRegistry
{
    if (gSharedRegistry == nil) {
        gSharedRegistry = [ORKParserRegistry new];
...
- (void)autoconfigure
{
    ...
    // JSON
    NSSet *JSONParserClassNames = [NSSet setWithObjects:@"ORKJSONParserJSONKit", @"ORKJSONParserYAJL", @"ORKJSONParserSBJSON", @"ORKJSONParserNXJSON", nil];
    for (NSString *parserClassName in JSONParserClassNames) {
        parserClass = NSClassFromString(parserClassName);
        if (parserClass) {
            [self setParserClass:parserClass forMIMEType:ORKMIMETypeJSON];
            break;
        }
    }

    // XML
    parserClass = NSClassFromString(@"ORKXMLParserXMLReader");
    if (parserClass) {
        [self setParserClass:parserClass forMIMEType:ORKMIMETypeXML];
        [self setParserClass:parserClass forMIMEType:ORKMIMETypeTextXML];
    }
}
