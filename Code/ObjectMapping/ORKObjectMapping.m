...
#import "ORKLog.h"
#import "ORKISO8601DateFormatter.h"

// Constants
NSString * const ORKObjectMappingNestingAttributeKeyName = @"<ORK_NESTING_ATTRIBUTE>";

@implementation ORKObjectMapping
...
{
    ORKObjectMapping *mapping = [self new];
...
+ (id)mappingForClass:(Class)objectClass usingBlock:(void (^)(ORKObjectMapping *))block
{
    ORKObjectMapping *mapping = [self mappingForClass:objectClass];
    block(mapping);
    return mapping;
}

+ (id)serializationMappingUsingBlock:(void (^)(ORKObjectMapping *))block
{
    ORKObjectMapping *mapping = [self serializationMapping];
    block(mapping);
    return mapping;
}

// Deprecated... Move to category or bottom...
+ (id)mappingForClass:(Class)objectClass withBlock:(void (^)(ORKObjectMapping *))block
{
    return [self mappingForClass:objectClass usingBlock:block];
}

+ (id)mappingForClass:(Class)objectClass block:(void (^)(ORKObjectMapping *))block
{
    return [self mappingForClass:objectClass usingBlock:block];
}

+ (id)serializationMappingWithBlock:(void (^)(ORKObjectMapping *))block
{
    ORKObjectMapping *mapping = [self serializationMapping];
    block(mapping);
    return mapping;
...
{
    ORKObjectMapping *copy = [[[self class] allocWithZone:zone] init];
    ...
    for (ORKObjectAttributeMapping *mapping in self.mappings) {
...
- (NSArray *)attributeMappings
{
    ...
    for (ORKObjectAttributeMapping *mapping in self.mappings) {
        if ([mapping isMemberOfClass:[ORKObjectAttributeMapping class]]) {
...
- (NSArray *)relationshipMappings
{
    NSMutableArray *mappings = [NSMutableArray array];
    for (ORKObjectAttributeMapping *mapping in self.mappings) {
        if ([mapping isMemberOfClass:[ORKObjectRelationshipMapping class]]) {
            [mappings addObject:mapping];
        }
    }
...
{
    for (ORKObjectAttributeMapping *mapping in _mappings) {
...
- (void)mapAttributesCollection:(id<NSFastEnumeration>)attributes
{
    for (NSString *attributeKeyPath in attributes) {
        [self addAttributeMapping:[ORKObjectAttributeMapping mappingFromKeyPath:attributeKeyPath toKeyPath:attributeKeyPath]];
    }
}
...
{
    ORKObjectRelationshipMapping *mapping = [ORKObjectRelationshipMapping mappingFromKeyPath:relationshipKeyPath toKeyPath:keyPath withMapping:objectOrDynamicMapping reversible:serialize];
    [self addRelationshipMapping:mapping];
}

- (void)mapKeyPath:(NSString *)relationshipKeyPath toRelationship:(NSString *)keyPath withMapping:(ORKObjectMappingDefinition *)objectOrDynamicMapping
{
    [self mapKeyPath:relationshipKeyPath toRelationship:keyPath withMapping:objectOrDynamicMapping serialize:YES];
}

- (void)mapRelationship:(NSString *)relationshipKeyPath withMapping:(ORKObjectMappingDefinition *)objectOrDynamicMapping
{
...
{
    ORKObjectAttributeMapping *mapping = [ORKObjectAttributeMapping mappingFromKeyPath:sourceKeyPath toKeyPath:destinationKeyPath];
    [self addAttributeMapping:mapping];
}
...
{
    ORKObjectAttributeMapping *mapping = [self mappingForKeyPath:keyPath];
...
#endif
- (ORKObjectMapping *)inverseMappingAtDepth:(NSInteger)depth
{
    ...
    ORKObjectMapping *inverseMapping = [ORKObjectMapping mappingForClass:[NSMutableDictionary class]];
    ...
    for (ORKObjectRelationshipMapping *relationshipMapping in self.relationshipMappings) {
        if (relationshipMapping.reversible) {
            ...
            if (! [mapping isKindOfClass:[ORKObjectMapping class]]) {
                ORKLogWarning(@"Unable to generate inverse mapping for relationship '%@': %@ relationships cannot be inversed.", relationshipMapping.sourceKeyPath, NSStringFromClass([mapping class]));
                continue;
            }
            [inverseMapping mapKeyPath:relationshipMapping.destinationKeyPath toRelationship:relationshipMapping.sourceKeyPath withMapping:[(ORKObjectMapping *)mapping inverseMappingAtDepth:depth+1]];
        }
    }
...
{
    [self mapKeyPath:ORKObjectMappingNestingAttributeKeyName toAttribute:attributeName];
}

- (ORKObjectAttributeMapping *)attributeMappingForKeyOfNestedDictionary
{
    return [self mappingForKeyPath:ORKObjectMappingNestingAttributeKeyName];
}

- (ORKObjectAttributeMapping *)mappingForAttribute:(NSString *)attributeKey
{
    for (ORKObjectAttributeMapping *mapping in [self attributeMappings]) {
...
{
    for (ORKObjectRelationshipMapping *mapping in [self relationshipMappings]) {
...
{
    return [[ORKObjectPropertyInspector sharedInspector] typeForProperty:propertyName ofClass:self.objectClass];
}
...
{
    return _preferredDateFormatter ? _preferredDateFormatter : [ORKObjectMapping preferredDateFormatter];
}

- (NSArray *)dateFormatters
{
    return _dateFormatters ? _dateFormatters : [ORKObjectMapping defaultDateFormatters];
}
...
@implementation ORKObjectMapping (DateAndTimeFormatting)
...
+ (NSArray *)defaultDateFormatters
{
    if (!defaultDateFormatters) {
        ...
        ORKISO8601DateFormatter *isoFormatter = [[ORKISO8601DateFormatter alloc] init];
