...
@class ORKObjectMappingOperation;
@class ORKMappingOperationQueue;

/**
 Objects acting as the delegate for ORKObjectMappingOperation objects must adopt the
 ORKObjectMappingOperationDelegate protocol. These methods enable the delegate to be
 notified of events such as the application of attribute and relationship mappings
 during a mapping operation.
 */
@protocol ORKObjectMappingOperationDelegate  <NSObject>
...
/**
 ...
 @param mapping The ORKObjectAttributeMapping or ORKObjectRelationshipMapping found for the key path.
 @param keyPath The key path in the source object for which the mapping is to be applied.
 */
- (void)objectMappingOperation:(ORKObjectMappingOperation *)operation didFindMapping:(ORKObjectAttributeMapping *)mapping forKeyPath:(NSString *)keyPath;
 ...
 */
- (void)objectMappingOperation:(ORKObjectMappingOperation *)operation didNotFindMappingForKeyPath:(NSString *)keyPath;
 ...
 */
- (void)objectMappingOperation:(ORKObjectMappingOperation *)operation didFailWithError:(NSError *)error;
@end

/**
 Instances of ORKObjectMappingOperation perform transformation between object
 representations according to the rules express in ORKObjectMapping objects. Mapping
 ...
 */
@interface ORKObjectMappingOperation : NSObject
 ...
 */
@property (nonatomic, assign) id<ORKObjectMappingOperationDelegate> delegate;

/**
 An operation queue for deferring portions of the mapping process until later

 Defaults to nil. If this mapping operation was configured by an instance of ORKObjectMapper, then
 ...
 */
@property (nonatomic, retain) ORKMappingOperationQueue *queue;
...
/**
 ...
 Note that if Core Data support is available, an instance of ORKManagedObjectMappingOperation may be returned.
 ...
 @param mapping An instance of ORKObjectMapping or ORKDynamicObjectMapping defining how the
 mapping is to be performed.
 @return An instance of ORKObjectMappingOperation or ORKManagedObjectMappingOperation for performing the mapping.
 */
+ (id)mappingOperationFromObject:(id)sourceObject toObject:(id)destinationObject withMapping:(ORKObjectMappingDefinition *)mapping;
 ...
 */
- (id)initWithSourceObject:(id)sourceObject destinationObject:(id)destinationObject mapping:(ORKObjectMappingDefinition *)mapping;
