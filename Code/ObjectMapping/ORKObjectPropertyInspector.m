...
static ORKObjectPropertyInspector *sharedInspector = nil;

@implementation ORKObjectPropertyInspector

+ (ORKObjectPropertyInspector *)sharedInspector
{
    if (sharedInspector == nil) {
        sharedInspector = [ORKObjectPropertyInspector new];
    }
...
- (NSDictionary *)propertyNamesAndTypesForClass:(Class)theClass
{
    ...
    while (currentClass != nil) {
        ...
        for (i = 0; i < outCount; i++) {
            ...
            if (![propName isEqualToString:@"_mapkit_hasPanoramaID"]) {
                const char *className = [[ORKObjectPropertyInspector propertyTypeFromAttributeString:attributeString] cStringUsingEncoding:NSUTF8StringEncoding];
    ...
    ORKLogDebug(@"Cached property names and types for Class '%@': %@", NSStringFromClass(theClass), propertyNames);
    return propertyNames;
}
