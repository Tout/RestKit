...
#import "ORKObjectLoader_Internals.h"
#import "ORKParserRegistry.h"
#import "ORKRequest_Internals.h"
...
@implementation ORKObjectLoader
...
- (id)initWithURL:(ORKURL *)URL mappingProvider:(ORKObjectMappingProvider *)mappingProvider
{
    ...
    if (self) {
        ...
        _mappingQueue = [ORKObjectManager defaultMappingQueue];
    }
...
{
    [(NSObject<ORKObjectLoaderDelegate>*)_delegate objectLoader:self didFailWithError:error];
...
// ORKRequestQueue to remove requests from the queue. All requests need to be finalized.
- (void)finalizeLoad:(BOOL)successful
{
    ...
    if ([self.delegate respondsToSelector:@selector(objectLoaderDidFinishLoading:)]) {
        [(NSObject<ORKObjectLoaderDelegate>*)self.delegate performSelectorOnMainThread:@selector(objectLoaderDidFinishLoading:)
...
- (void)informDelegateOfObjectLoadWithResultDictionary:(NSDictionary *)resultDictionary
{
    ...
    ORKObjectMappingResult *result = [ORKObjectMappingResult mappingResultWithDictionary:resultDictionary];

    // Dictionary callback
    if ([self.delegate respondsToSelector:@selector(objectLoader:didLoadObjectDictionary:)]) {
        [(NSObject<ORKObjectLoaderDelegate>*)self.delegate objectLoader:self didLoadObjectDictionary:[result asDictionary]];
    }
    ...
    if ([self.delegate respondsToSelector:@selector(objectLoader:didLoadObjects:)]) {
        [(NSObject<ORKObjectLoaderDelegate>*)self.delegate objectLoader:self didLoadObjects:[result asCollection]];
    }
    ...
    if ([self.delegate respondsToSelector:@selector(objectLoader:didLoadObject:)]) {
        [(NSObject<ORKObjectLoaderDelegate>*)self.delegate objectLoader:self didLoadObject:[result asObject]];
    }
...
/**
 Overloaded by ORKManagedObjectLoader to serialize/deserialize managed objects
 at thread boundaries.
 @protected
 */
- (void)processMappingResult:(ORKObjectMappingResult *)result
{
...
- (ORKObjectMappingResult *)mapResponseWithMappingProvider:(ORKObjectMappingProvider *)mappingProvider toObject:(id)targetObject inContext:(ORKObjectMappingProviderContext)context error:(NSError **)error
{
    id<ORKParser> parser = [[ORKParserRegistry sharedRegistry] parserForMIMEType:self.response.MIMEType];
    ...
    ORKLogTrace(@"bodyAsString: %@", bodyAsString);
    if (bodyAsString == nil || [[bodyAsString stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] length] == 0) {
        ORKLogDebug(@"Mapping attempted on empty response body...");
        if (self.targetObject) {
            return [ORKObjectMappingResult mappingResultWithDictionary:[NSDictionary dictionaryWithObject:self.targetObject forKey:@""]];
        }

        return [ORKObjectMappingResult mappingResultWithDictionary:[NSDictionary dictionary]];
    }
    ...
    if ([self.delegate respondsToSelector:@selector(objectLoader:willMapData:)]) {
        ...
        [(NSObject<ORKObjectLoaderDelegate>*)self.delegate objectLoader:self willMapData:&parsedData];
    }
    ...
    if (mapper.errorCount > 0) {
        ORKLogError(@"Encountered errors during mapping: %@", [[mapper.errors valueForKey:@"localizedDescription"] componentsJoinedByString:@", "]);
    }
...
- (ORKObjectMappingDefinition *)configuredObjectMapping
{
...
- (ORKObjectMappingResult *)performMapping:(NSError **)error
{
    ...
    ORKObjectMappingDefinition *configuredObjectMapping = [self configuredObjectMapping];
    if (configuredObjectMapping) {
        mappingProvider = [ORKObjectMappingProvider mappingProvider];
        ...
        ORKLogDebug(@"No object mapping provider, using mapping provider from parent object manager to perform KVC mapping");
        mappingProvider = self.mappingProvider;
    }

    return [self mapResponseWithMappingProvider:mappingProvider toObject:self.targetObject inContext:ORKObjectMappingProviderContextObjectsByKeyPath error:error];
}
...
- (void)performMappingInDispatchQueue
{
    ...
    dispatch_async(self.mappingQueue, ^{
        ...
        ORKLogDebug(@"Beginning object mapping activities within GCD queue labeled: %s", dispatch_queue_get_label(self.mappingQueue));
        NSError *error = nil;
...
{
    if ([[ORKParserRegistry sharedRegistry] parserForMIMEType:self.response.MIMEType]) {
        return YES;
    }

    ORKLogWarning(@"Unable to find parser for MIME Type '%@'", MIMEType);
    return NO;
}
...
- (BOOL)isResponseMappable
{
    ...
    if ([self.response isFailure]) {
        ...
        ORKLogWarning(@"Encountered unexpected response with status code: %ld (MIME Type: %@ -> URL: %@)", (long)self.response.statusCode, self.response.MIMEType, self.URL);
        NSError *error = [NSError errorWithDomain:ORKErrorDomain code:ORKObjectLoaderUnexpectedResponseError userInfo:nil];
        if ([_delegate respondsToSelector:@selector(objectLoaderDidLoadUnexpectedResponse:)]) {
            [(NSObject<ORKObjectLoaderDelegate>*)_delegate objectLoaderDidLoadUnexpectedResponse:self];
        } else {
...
- (void)handleResponseError
{
    ...
    // target object
    NSError *error = nil;
    ORKObjectMappingResult *result = [self mapResponseWithMappingProvider:self.mappingProvider toObject:nil inContext:ORKObjectMappingProviderContextErrors error:&error];
    if (result) {
        error = [result asError];
    } else {
        ORKLogError(@"Encountered an error while attempting to map server side errors from payload: %@", [error localizedDescription]);
    }
...
#pragma mark - ORKRequest & ORKRequestDelegate methods

// Invoked just before request hits the network
- (BOOL)prepareURLRequest
{
    if ((self.sourceObject && self.params == nil) && (self.method == ORKRequestMethodPOST || self.method == ORKRequestMethodPUT)) {
        ...
        ORKLogDebug(@"POST or PUT request for source object %@, serializing to MIME Type %@ for transport...", self.sourceObject, self.serializationMIMEType);
        ORKObjectSerializer *serializer = [ORKObjectSerializer serializerWithObject:self.sourceObject mapping:self.serializationMapping];
        NSError *error = nil;
        id params = [serializer serializationForMIMEType:self.serializationMIMEType error:&error];

        if (error) {
            ORKLogError(@"Serializing failed for source object %@ to MIME Type %@: %@", self.sourceObject, self.serializationMIMEType, [error localizedDescription]);
...
//    NSAssert([NSThread isMainThread], @"ORKObjectLoaderDelegate callbacks must occur on the main thread");
    ...
    if (![self.response wasLoadedFromCache] && [self.response isSuccessful] && (_cachePolicy != ORKRequestCachePolicyNone)) {
...
// really not be a subclass of ORKRequest.
- (void)setDelegate:(id<ORKObjectLoaderDelegate>)delegate
{
    [super setDelegate:delegate];
}

- (id<ORKObjectLoaderDelegate>)delegate
{
    return (id<ORKObjectLoaderDelegate>) [super delegate];
}

@end

@implementation ORKObjectLoader (Deprecations)

+ (id)loaderWithResourcePath:(NSString *)resourcePath objectManager:(ORKObjectManager *)objectManager delegate:(id<ORKObjectLoaderDelegate>)delegate
{
    return [[[self alloc] initWithResourcePath:resourcePath objectManager:objectManager delegate:delegate] autorelease];
}

- (id)initWithResourcePath:(NSString *)resourcePath objectManager:(ORKObjectManager *)objectManager delegate:(id<ORKObjectLoaderDelegate>)theDelegate
{
