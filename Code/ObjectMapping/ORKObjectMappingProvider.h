...
// @see ORKObjectMappingProvider+Contexts.h
typedef enum {
    ORKObjectMappingProviderContextObjectsByKeyPath = 1000,
    ORKObjectMappingProviderContextObjectsByType,
    ORKObjectMappingProviderContextObjectsByResourcePathPattern,
    ORKObjectMappingProviderContextSerialization,
    ORKObjectMappingProviderContextErrors,
    ORKObjectMappingProviderContextPagination
} ORKObjectMappingProviderContext;

/**
 The mapping provider is a repository of registered object mappings for use by instances
 of ORKObjectManager and ORKObjectMapper. It provides for the storage and retrieval of object
 mappings by keyPath and type.

 The mapping provider is responsible for:

 1. Providing instances of ORKObjectMapper with keyPaths and object mappings for use
    ...
    found, the ORKObjectMapper will instantiate an ORKObjectMappingOperation to perform the mapping
    using the ORKObjectMapping or ORKDynamicObjectMapping associated with the keyPath.
 1. Providing the appropriate serialization mapping to instances of ORKObjectManager when an object
    is to be sent to the remote server using [ORKObjectManager postObject:delegate:] or
    [ORKObjectManager postObject:delegate]. This mapping is used to serialize the object into a
    format suitable for encoding into a URL form encoded or JSON representation.
 1. Providing convenient storage of ORKObjectMapping references for users who are not using keyPath
    based mapping. Mappings can be added to the provider and retrieved by the [ORKObjectMapping objectClass]
    that they target.
 */
@interface ORKObjectMappingProvider : NSObject {
    NSMutableDictionary *mappingContexts;
}

/**
 Creates and returns an autoreleased ORKObjectMappingProvider instance.
 ...
 */
+ (id)mappingProviderUsingBlock:(void (^)(ORKObjectMappingProvider *))block;

/**
 Configures the mapping provider to use the ORKObjectMapping or ORKDynamicObjectMapping provided when
 content is encountered at the specified keyPath.

 When an ORKObjectMapper is performing its work, each registered keyPath within the mapping provider will
 be searched for content in the parsed payload. If mappable content is found, the object mapping configured
 for the keyPath will be used to perform an ORKObjectMappingOperation.

 @param objectOrDynamicMapping An ORKObjectMapping or ORKDynamicObjectMapping to register for keyPath based mapping.
 @param keyPath The keyPath to register the mapping as being responsible for mapping.
 @see ORKObjectMapper
 @see ORKObjectMappingOperation
 */
- (void)setObjectMapping:(ORKObjectMappingDefinition *)objectOrDynamicMapping forKeyPath:(NSString *)keyPath;

/**
 Returns the ORKObjectMapping or ORKObjectDynamic mapping configured for use
 when mappable content is encountered at keyPath

 @param keyPath A registered keyPath to retrieve the object mapping for
 @return The ORKObjectMapping or ORKDynamicObjectMapping for the specified keyPath or nil if none is registered.
 */
- (ORKObjectMappingDefinition *)objectMappingForKeyPath:(NSString *)keyPath;

/**
 Removes the ORKObjectMapping or ORKDynamicObjectMapping registered at the specified keyPath
 from the provider.
...
/**
 Returns a dictionary where the keys are mappable keyPaths and the values are the ORKObjectMapping
 or ORKDynamicObjectMapping to use for mappable data that appears at the keyPath.

 @warning The returned dictionary can contain ORKDynamicObjectMapping instances. Check the type if
...
/**
 ...
 We might configure a mapping like so:
    ...
    ORKObjectMapping *mapping = [ORKObjectMapping mappingForClass:[Person class]];
 ...
 we can auto-generate the serialization mapping and set the whole thing up in one shot:
    ...
    [[ORKObjectManager sharedManager].mappingProvider registerMapping:mapping withRootKeyPath:@"person"];
 ...
 If you want to manipulate the serialization mapping yourself, you can work with the mapping directly:
    ...
    ORKObjectMapping *serializationMappingForPerson = [personMapping inverseMapping];
    // NOTE: Serialization mapping default to a nil root keyPath and will serialize to a flat dictionary
    [[ORKObjectManager sharedManager].mappingProvider setSerializationMapping:serializationMappingForPerson forClass:[Person class]];
...
/**
 ...
 must be explicitly set on an instance of ORKObjectLoader or ORKObjectMappingOperation to be applied. This is useful
...
/**
 ...
 might have an objectClass of ORKTweet for storing Tweet data. We could retrieve
 the object mapping for this model by invoking
 `[mappingProvider objectMappingForClass:[ORKTweet class]];`
 ...
 @return An ORKObjectMapping matching objectClass or nil
 */
- (ORKObjectMapping *)objectMappingForClass:(Class)objectClass;
...
/**
 ...
 Serialization mappings are simply instances of ORKObjectMapping that target NSMutableDictionary
 ...
 @see [ORKObjectMapping serializationMapping]
 */
- (void)setSerializationMapping:(ORKObjectMapping *)objectMapping forClass:(Class)objectClass;
...
/**
 ...
 @return The ORKObjectMapping instance used for mapping instances of objectClass for transport
...
/**
 ...
 the ORKObjectLoader instance matches resourcePathPattern.
 ...
 **NOTE** that care must be taken when configuring patterns within the provider. The patterns
 ...
 @param objectMapping The object mapping to use when the resourcePath matches the specified
    resourcePathPattern.
 @param resourcePathPattern A pattern to be evaluated using an ORKPathMatcher against a resourcePath
    to determine if objectMapping is the appropriate mapping.
 @see ORKPathMatcher
 @see ORKURL
 @see ORKObjectLoader
 */
- (void)setObjectMapping:(ORKObjectMappingDefinition *)objectMapping forResourcePathPattern:(NSString *)resourcePathPattern;

/**
 Returns the first objectMapping configured in the provider with a resourcePathPattern matching
 the specified resourcePath.

 @param resourcePath A resource path to retrieve the first ORKObjectMapping or ORKDynamicObjectMapping
    configured with a matching pattern.
 @return An ORKObjectMapping or ORKDynamicObjectMapping for a resource path pattern matching resourcePath
...
/**
 ...
 @see ORKObjectLoader
 @see ORKParserRegistry
 */
@property (nonatomic, retain) ORKObjectMapping *errorMapping;
 ...
 */
@property (nonatomic, retain) ORKObjectMapping *paginationMapping;

@end

// Method signatures being phased out
@interface ORKObjectMappingProvider (CompatibilityAliases)
...
- (void)setMapping:(ORKObjectMappingDefinition *)objectOrDynamicMapping forKeyPath:(NSString *)keyPath;
- (ORKObjectMappingDefinition *)mappingForKeyPath:(NSString *)keyPath;
