...
@implementation ORKObjectMapper
...
+ (id)mapperWithObject:(id)object mappingProvider:(ORKObjectMappingProvider *)theMappingProvider
{
    return [[[self alloc] initWithObject:object mappingProvider:theMappingProvider] autorelease];
}

- (id)initWithObject:(id)object mappingProvider:(ORKObjectMappingProvider *)theMappingProvider
{
    self = [super init];
    if (self) {
        ...
        operationQueue = [ORKMappingOperationQueue new];
        context = ORKObjectMappingProviderContextObjectsByKeyPath;
    }
...
- (void)addError:(NSError *)error
{
    ...
    ORKLogWarning(@"Adding mapping error: %@", [error localizedDescription]);
}

- (void)addErrorWithCode:(ORKObjectMapperErrorCode)errorCode message:(NSString *)errorMessage keyPath:(NSString *)keyPath userInfo:(NSDictionary *)otherInfo
{
    NSMutableDictionary *userInfo = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                     errorMessage, NSLocalizedDescriptionKey,
                                     @"ORKObjectMapperKeyPath", keyPath ? keyPath : (NSString *)[NSNull null],
                                     nil];
    [userInfo addEntriesFromDictionary:otherInfo];
    NSError *error = [NSError errorWithDomain:ORKErrorDomain code:errorCode userInfo:userInfo];
    [self addError:error];
}
...
- (void)addErrorForUnmappableKeyPath:(NSString *)keyPath
{
    ...
    [self addErrorWithCode:ORKObjectMapperErrorObjectMappingNotFound message:errorMessage keyPath:keyPath userInfo:nil];
}
...
- (BOOL)isNullCollection:(id)object
{
    ...
    if ([object respondsToSelector:@selector(countForObject:)] && [object count] > 0) {
        if ([object countForObject:[NSNull null]] == [object count]) {
            ORKLogDebug(@"Found a collection containing only NSNull values, considering the collection unmappable...");
            return YES;
        }
    }
...
- (id)mapObject:(id)mappableObject atKeyPath:(NSString *)keyPath usingMapping:(ORKObjectMappingDefinition *)mapping
{
    ...
    if (self.targetObject) {
        ...
        if ([mapping isKindOfClass:[ORKDynamicObjectMapping class]]) {
            objectMapping = [(ORKDynamicObjectMapping *)mapping objectMappingForDictionary:mappableObject];
        ...
        if (NO == [[self.targetObject class] isSubclassOfClass:objectMapping.objectClass]) {
            ...
            [self addErrorWithCode:ORKObjectMapperErrorObjectMappingTypeMismatch message:errorMessage keyPath:keyPath userInfo:nil];
            return nil;
        }
    } else {
...
- (NSArray *)mapCollection:(NSArray *)mappableObjects atKeyPath:(NSString *)keyPath usingMapping:(ORKObjectMappingDefinition *)mapping
{
    ...
    if (mapping.forceCollectionMapping) {
        ...
        if ([mappableObjects isKindOfClass:[NSDictionary class]]) {
            ...
            ORKLogWarning(@"Collection mapping forced but mappable objects is of type '%@' rather than NSDictionary", NSStringFromClass([mappableObjects class]));
        }
    }
...
- (BOOL)mapFromObject:(id)mappableObject toObject:(id)destinationObject atKeyPath:(NSString *)keyPath usingMapping:(ORKObjectMappingDefinition *)mapping
{
    ...
    ORKLogDebug(@"Asked to map source object %@ with mapping %@", mappableObject, mapping);
    ...
    ORKObjectMappingOperation *operation = [ORKObjectMappingOperation mappingOperationFromObject:mappableObject
...
- (id)objectWithMapping:(ORKObjectMappingDefinition *)mapping andData:(id)mappableData
{
    NSAssert([mapping isKindOfClass:[ORKObjectMappingDefinition class]], @"Expected an ORKObjectMappingDefinition object");
    ORKObjectMapping *objectMapping = nil;
    if ([mapping isKindOfClass:[ORKDynamicObjectMapping class]]) {
        objectMapping = [(ORKDynamicObjectMapping *)mapping objectMappingForDictionary:mappableData];
        if (! objectMapping) {
            ORKLogDebug(@"Mapping %@ declined mapping for data %@: returned nil objectMapping", mapping, mappableData);
        }
    } else if ([mapping isKindOfClass:[ORKObjectMapping class]]) {
        objectMapping = (ORKObjectMapping *)mapping;
    } else {
...
- (id)performMappingForObject:(id)mappableValue atKeyPath:(NSString *)keyPath usingMapping:(ORKObjectMappingDefinition *)mapping
{
    id mappingResult;
    if (mapping.forceCollectionMapping || [mappableValue isKindOfClass:[NSArray class]] || [mappableValue isKindOfClass:[NSSet class]]) {
        ORKLogDebug(@"Found mappable collection at keyPath '%@': %@", keyPath, mappableValue);
        mappingResult = [self mapCollection:mappableValue atKeyPath:keyPath usingMapping:mapping];
    } else {
        ORKLogDebug(@"Found mappable data at keyPath '%@': %@", keyPath, mappableValue);
...
- (NSMutableDictionary *)performKeyPathMappingUsingMappingDictionary:(NSDictionary *)mappingsByKeyPath
{
    ...
    for (NSString *keyPath in mappingsByKeyPath) {
        ...
        ORKLogTrace(@"Examining keyPath '%@' for mappable content...", keyPath);
        ...
        if (mappableValue == nil || mappableValue == [NSNull null] || [self isNullCollection:mappableValue]) {
            ORKLogDebug(@"Found unmappable value at keyPath: %@", keyPath);
        ...
        foundMappable = YES;
        ORKObjectMappingDefinition *mapping = [mappingsByKeyPath objectForKey:keyPath];
...
- (ORKObjectMappingResult *)performMapping
{
    ...
    if ([mappingsForContext isKindOfClass:[NSDictionary class]]) {
    ...
    } else if ([mappingsForContext isKindOfClass:[ORKObjectMappingDefinition class]]) {
        ...
        if ([mappingsForContext rootKeyPath] != nil) {
            ...
            ORKLogDebug(@"Selected object mapping has rootKeyPath. Apply valueForKeyPath to mappable data: %@", rootKeyPath);
        }
    ...
    ORKLogDebug(@"The following operations are in the queue: %@", operationQueue.operations);
    ...
    ORKLogDebug(@"Finished performing object mapping. Results: %@", results);

    return [ORKObjectMappingResult mappingResultWithDictionary:results];
}
