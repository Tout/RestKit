...
+ (id)mappingOperationFromObject:(id)sourceObject toObject:(id)destinationObject withMapping:(ORKObjectMappingDefinition *)objectMapping
{
    // Check for availability of ManagedObjectMappingOperation. Better approach for handling?
    Class targetClass = NSClassFromString(@"ORKManagedObjectMappingOperation");
    if (targetClass == nil) targetClass = [ORKObjectMappingOperation class];
...
- (id)initWithSourceObject:(id)sourceObject destinationObject:(id)destinationObject mapping:(ORKObjectMappingDefinition *)objectMapping
{
    ...
    if (self) {
        ...
        if ([objectMapping isKindOfClass:[ORKDynamicObjectMapping class]]) {
            _objectMapping = [[(ORKDynamicObjectMapping *)objectMapping objectMappingForDictionary:_sourceObject] retain];
            ORKLogDebug(@"ORKObjectMappingOperation was initialized with a dynamic mapping. Determined concrete mapping = %@", _objectMapping);
        } else if ([objectMapping isKindOfClass:[ORKObjectMapping class]]) {
            _objectMapping = (ORKObjectMapping *)[objectMapping retain];
        }
...
{
    ORKLogTrace(@"Transforming string value '%@' to NSDate...", string);
    ...
    if (numeric) {
        ...
        @synchronized(dateFormatter) {
                if ([dateFormatter isKindOfClass:[NSDateFormatter class]]) {
                    ORKLogTrace(@"Attempting to parse string '%@' with format string '%@' and time zone '%@'", string, [(NSDateFormatter *)dateFormatter dateFormat], [(NSDateFormatter *)dateFormatter timeZone]);
                }
        ...
        if (success && date) {
                if ([dateFormatter isKindOfClass:[NSDateFormatter class]]) {
                    ORKLogTrace(@"Successfully parsed string '%@' with format string '%@' and time zone '%@' and turned into date '%@'",
...
{
    ORKLogTrace(@"Found transformable value at keyPath '%@'. Transforming from type '%@' to '%@'", keyPath, NSStringFromClass([value class]), NSStringFromClass(destinationType));
    ...
    ORKLogWarning(@"Failed transformation of value at keyPath '%@'. No strategy for transforming from '%@' to '%@'", keyPath, NSStringFromClass([value class]), NSStringFromClass(destinationType));
...
{
    return ORKObjectIsValueEqualToValue(sourceValue, destinationValue);
}
...
- (BOOL)validateValue:(id *)value atKeyPath:(NSString *)keyPath
{
    ...
    if (self.objectMapping.performKeyValueValidation && [self.destinationObject respondsToSelector:@selector(validateValue:forKeyPath:error:)]) {
        ...
        if (!success) {
            if (_validationError) {
                ORKLogError(@"Validation failed while mapping attribute at key path '%@' to value %@. Error: %@", keyPath, *value, [_validationError localizedDescription]);
                ORKLogValidationError(_validationError);
            } else {
                ORKLogWarning(@"Destination object %@ rejected attribute value %@ for keyPath %@. Skipping...", self.destinationObject, *value, keyPath);
            }
        }
    }
...
- (BOOL)shouldSetValue:(id *)value atKeyPath:(NSString *)keyPath
{
    ...
    if (*value == [NSNull null] || [*value isEqual:[NSNull null]]) {
        ORKLogWarning(@"Coercing NSNull value to nil in shouldSetValue:atKeyPath: -- should be fixed.");
        *value = nil;
    }
...
- (NSArray *)applyNestingToMappings:(NSArray *)mappings
{
    if (_nestedAttributeSubstitution) {
        ...
        for (ORKObjectAttributeMapping *mapping in mappings) {
            ORKObjectAttributeMapping *nestedMapping = [mapping copy];
...
- (void)applyAttributeMapping:(ORKObjectAttributeMapping *)attributeMapping withValue:(id)value
{
    ...
    ORKLogTrace(@"Mapping attribute value keyPath '%@' to '%@'", attributeMapping.sourceKeyPath, attributeMapping.destinationKeyPath);
    ...
    if ([self shouldSetValue:&value atKeyPath:attributeMapping.destinationKeyPath]) {
        ORKLogTrace(@"Mapped attribute value from keyPath '%@' to '%@'. Value: %@", attributeMapping.sourceKeyPath, attributeMapping.destinationKeyPath, value);
        ...
        ORKLogTrace(@"Skipped mapping of attribute value from keyPath '%@ to keyPath '%@' -- value is unchanged (%@)", attributeMapping.sourceKeyPath, attributeMapping.destinationKeyPath, value);
...
- (BOOL)applyAttributeMappings
{
    ...
    if (!self.objectMapping.performKeyValueValidation) {
        ORKLogDebug(@"Key-value validation is disabled for mapping, skipping...");
    }

    for (ORKObjectAttributeMapping *attributeMapping in [self attributeMappings]) {
        if ([attributeMapping isMappingForKeyOfNestedDictionary]) {
            ORKLogTrace(@"Skipping attribute mapping for special keyPath '%@'", attributeMapping.sourceKeyPath);
            continue;
        }
        ...
        if (self.objectMapping.ignoreUnknownKeyPaths && ![self.sourceObject respondsToSelector:NSSelectorFromString(attributeMapping.sourceKeyPath)]) {
            ORKLogDebug(@"Source object is not key-value coding compliant for the keyPath '%@', skipping...", attributeMapping.sourceKeyPath);
            continue;
        }
        ...
        @catch (NSException *exception) {
            if ([[exception name] isEqualToString:NSUndefinedKeyException] && self.objectMapping.ignoreUnknownKeyPaths) {
                ORKLogWarning(@"Encountered an undefined attribute mapping for keyPath '%@' that generated NSUndefinedKeyException exception. Skipping due to objectMapping.ignoreUnknownKeyPaths = YES",
        ...
        if (value) {
            ...
            ORKLogTrace(@"Did not find mappable attribute value keyPath '%@'", attributeMapping.sourceKeyPath);
            ...
            if ([self.objectMapping shouldSetDefaultValueForMissingAttributes]) {
                ...
                ORKLogTrace(@"Setting nil for missing attribute value at keyPath '%@'", attributeMapping.sourceKeyPath);
            }
        }
...
- (BOOL)mapNestedObject:(id)anObject toObject:(id)anotherObject withRealtionshipMapping:(ORKObjectRelationshipMapping *)relationshipMapping
{
    ...
    ORKLogTrace(@"Performing nested object mapping using mapping %@ for data: %@", relationshipMapping, anObject);
    ORKObjectMappingOperation *subOperation = [ORKObjectMappingOperation mappingOperationFromObject:anObject toObject:anotherObject withMapping:relationshipMapping.mapping];
    subOperation.delegate = self.delegate;
    subOperation.queue = self.queue;
    if (NO == [subOperation performMapping:&error]) {
        ORKLogWarning(@"WARNING: Failed mapping nested object: %@", [error localizedDescription]);
    }
...
- (BOOL)applyRelationshipMappings
{
    ...
    for (ORKObjectRelationshipMapping *relationshipMapping in [self relationshipMappings]) {
        id value = nil;
        @try {
        ...
        @catch (NSException *exception) {
            if ([[exception name] isEqualToString:NSUndefinedKeyException] && self.objectMapping.ignoreUnknownKeyPaths) {
                ORKLogWarning(@"Encountered an undefined relationship mapping for keyPath '%@' that generated NSUndefinedKeyException exception. Skipping due to objectMapping.ignoreUnknownKeyPaths = YES",
        ...
        if (value == nil || value == [NSNull null] || [value isEqual:[NSNull null]]) {
            ORKLogDebug(@"Did not find mappable relationship value keyPath '%@'", relationshipMapping.sourceKeyPath);
            ...
            if ([self.objectMapping setNilForMissingRelationships] && [self shouldSetValue:&nilReference atKeyPath:relationshipMapping.destinationKeyPath]) {
                ORKLogTrace(@"Setting nil for missing relationship value at keyPath '%@'", relationshipMapping.sourceKeyPath);
        ...
        if (relationshipMapping.mapping.forceCollectionMapping) {
            ...
            if ([value isKindOfClass:[NSDictionary class]]) {
                ORKLogDebug(@"Collection mapping forced for NSDictionary, mapping each key/value independently...");
                ...
                ORKLogWarning(@"Collection mapping forced but mappable objects is of type '%@' rather than NSDictionary", NSStringFromClass([value class]));
            }
        }
        ...
        if (mappingToCollection && ![self isValueACollection:value]) {
            Class orderedSetClass = NSClassFromString(@"NSOrderedSet");
            ORKLogDebug(@"Asked to map a single object into a collection relationship. Transforming to an instance of: %@", NSStringFromClass(relationshipType));
            if ([relationshipType isSubclassOfClass:[NSArray class]]) {
                ...
                ORKLogWarning(@"Failed to transform single object");
            }
        }

        if ([self isValueACollection:value]) {
            // One to many relationship
            ORKLogDebug(@"Mapping one to many relationship value at keyPath '%@' to '%@'", relationshipMapping.sourceKeyPath, relationshipMapping.destinationKeyPath);
            appliedMappings = YES;
            ...
            if ([self isValueACollection:collectionSanityCheckObject]) {
                ORKLogWarning(@"WARNING: Detected a relationship mapping for a collection containing another collection. This is probably not what you want. Consider using a KVC collection operator (such as @unionOfArrays) to flatten your mappable collection.");
                ORKLogWarning(@"Key path '%@' yielded collection containing another collection rather than a collection of objects: %@", relationshipMapping.sourceKeyPath, value);
            }
            for (id nestedObject in value) {
                ORKObjectMappingDefinition *mapping = relationshipMapping.mapping;
                ORKObjectMapping *objectMapping = nil;
                if ([mapping isKindOfClass:[ORKDynamicObjectMapping class]]) {
                    objectMapping = [(ORKDynamicObjectMapping *)mapping objectMappingForDictionary:nestedObject];
                    if (! objectMapping) {
                        ORKLogDebug(@"Mapping %@ declined mapping for data %@: returned nil objectMapping", mapping, nestedObject);
                        continue;
                    }
                } else if ([mapping isKindOfClass:[ORKObjectMapping class]]) {
                    objectMapping = (ORKObjectMapping *)mapping;
                } else {
            ...
            if ([self shouldSetValue:&destinationObject atKeyPath:relationshipMapping.destinationKeyPath]) {
                ...
                if (managedObjectClass && [self.destinationObject isKindOfClass:managedObjectClass]) {
                    ORKLogTrace(@"Found a managedObject collection. About to apply value via mutable[Set|Array]ValueForKey");
                    if ([destinationObject isKindOfClass:[NSSet class]]) {
                        ORKLogTrace(@"Mapped NSSet relationship object from keyPath '%@' to '%@'. Value: %@", relationshipMapping.sourceKeyPath, relationshipMapping.destinationKeyPath, destinationObject);
                        ...
                        ORKLogTrace(@"Mapped NSArray relationship object from keyPath '%@' to '%@'. Value: %@", relationshipMapping.sourceKeyPath, relationshipMapping.destinationKeyPath, destinationObject);
                        ...
                        ORKLogTrace(@"Mapped NSOrderedSet relationship object from keyPath '%@' to '%@'. Value: %@", relationshipMapping.sourceKeyPath, relationshipMapping.destinationKeyPath, destinationObject);
                        [self.destinationObject setValue:destinationObject forKey:relationshipMapping.destinationKeyPath];
                    }
                } else {
                    ORKLogTrace(@"Mapped relationship object from keyPath '%@' to '%@'. Value: %@", relationshipMapping.sourceKeyPath, relationshipMapping.destinationKeyPath, destinationObject);
            ...
            ORKLogDebug(@"Mapping one to one relationship value at keyPath '%@' to '%@'", relationshipMapping.sourceKeyPath, relationshipMapping.destinationKeyPath);

            ORKObjectMappingDefinition *mapping = relationshipMapping.mapping;
            ORKObjectMapping *objectMapping = nil;
            if ([mapping isKindOfClass:[ORKDynamicObjectMapping class]]) {
                objectMapping = [(ORKDynamicObjectMapping *)mapping objectMappingForDictionary:value];
            } else if ([mapping isKindOfClass:[ORKObjectMapping class]]) {
                objectMapping = (ORKObjectMapping *)mapping;
            }
            NSAssert(objectMapping, @"Encountered unknown mapping type '%@'", NSStringFromClass([mapping class]));
...
{
    ORKObjectAttributeMapping *attributeMapping = [self.objectMapping attributeMappingForKeyOfNestedDictionary];
    if (attributeMapping) {
        ORKLogDebug(@"Found nested mapping definition to attribute '%@'", attributeMapping.destinationKeyPath);
        id attributeValue = [[self.sourceObject allKeys] lastObject];
        if (attributeValue) {
            ORKLogDebug(@"Found nesting value of '%@' for attribute '%@'", attributeValue, attributeMapping.destinationKeyPath);
            ...
            ORKLogWarning(@"Unable to find nesting value for attribute '%@'", attributeMapping.destinationKeyPath);
        }
    }
}
...
{
    ORKLogDebug(@"Starting mapping operation...");
    ORKLogTrace(@"Performing mapping operation: %@", self);
    ...
    if ((mappedAttributes || mappedRelationships) && _validationError == nil) {
        ORKLogDebug(@"Finished mapping operation successfully...");
        return YES;
    }
    ...
    if (_validationError) {
        ...
        ORKLogError(@"Failed mapping operation: %@", [_validationError localizedDescription]);
    } else {
        // We did not find anything to do
        ORKLogDebug(@"Mapping operation did not find any mappable content");
    }
...
{
    return [NSString stringWithFormat:@"ORKObjectMappingOperation for '%@' object. Mapping values from object %@ to object %@ with object mapping %@",
