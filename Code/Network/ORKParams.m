 ...
 */
NSString * const kORKStringBoundary = @"0xKhTmLbOuNdArY";

@implementation ORKParams

+ (ORKParams *)params
{
    ORKParams *params = [[[ORKParams alloc] init] autorelease];
    return params;
}

+ (ORKParams *)paramsWithDictionary:(NSDictionary *)dictionary
{
    ORKParams *params = [[[ORKParams alloc] initWithDictionary:dictionary] autorelease];
    return params;
}
...
- (id)init
{
    ...
    if (self) {
        ...
        _footer       = [[[NSString stringWithFormat:@"--%@--\r\n", kORKStringBoundary] dataUsingEncoding:NSUTF8StringEncoding] retain];
...
{
    ORKParamsAttachment *attachment = [[ORKParamsAttachment alloc] initWithName:param value:value];
...
- (NSDictionary *)dictionaryOfPlainTextParams
{
    ...
    for (ORKParamsAttachment *attachment in _attachments)
...
- (ORKParamsAttachment *)setFile:(NSString *)filePath forParam:(NSString *)param
{
    ...
    ORKParamsAttachment *attachment = [[ORKParamsAttachment alloc] initWithName:param file:filePath];
    [_attachments addObject:attachment];
    [attachment release];
...
- (ORKParamsAttachment *)setData:(NSData *)data forParam:(NSString *)param
{
    ...
    NSParameterAssert(param);
    ORKParamsAttachment *attachment = [[ORKParamsAttachment alloc] initWithName:param data:data];
    [_attachments addObject:attachment];
    [attachment release];
...
- (ORKParamsAttachment *)setData:(NSData *)data MIMEType:(NSString *)MIMEType forParam:(NSString *)param
{
    ...
    NSParameterAssert(param);
    ORKParamsAttachment *attachment = [self setData:data forParam:param];
    if (MIMEType != nil) {
...
- (ORKParamsAttachment *)setData:(NSData *)data MIMEType:(NSString *)MIMEType fileName:(NSString *)fileName forParam:(NSString *)param
{
    NSParameterAssert(data);
    NSParameterAssert(param);
    ORKParamsAttachment *attachment = [self setData:data forParam:param];
...
- (ORKParamsAttachment *)setFile:(NSString *)filePath MIMEType:(NSString *)MIMEType fileName:(NSString *)fileName forParam:(NSString *)param
{
    NSParameterAssert(filePath);
    NSParameterAssert(param);
    ORKParamsAttachment *attachment = [self setFile:filePath forParam:param];
    if (MIMEType) {
        attachment.MIMEType = MIMEType;
    }
    if (fileName) {
...
#pragma mark ORKRequestSerializable methods

- (NSString *)HTTPHeaderValueForContentType
{
    return [NSString stringWithFormat:@"multipart/form-data; boundary=%@", kORKStringBoundary];
}
...
- (void)open
{
    ...
    ORKLogTrace(@"ORKParams stream opened...");
}
...
- (void)close
{
    if (_streamStatus != NSStreamStatusClosed) {
        ...
        ORKLogTrace(@"ORKParams stream closed. Releasing self.");
...
- (NSString *)MD5
{
    ...
    for (ORKParamsAttachment *attachment in self.attachments) {
