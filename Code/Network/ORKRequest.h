...
@class ORKRequestCache;

/**
 HTTP methods for requests
 */
typedef enum ORKRequestMethod {
    ORKRequestMethodInvalid = -1,
    ORKRequestMethodGET,
    ORKRequestMethodPOST,
    ORKRequestMethodPUT,
    ORKRequestMethodDELETE,
    ORKRequestMethodHEAD
} ORKRequestMethod;

NSString *ORKRequestMethodNameFromType(ORKRequestMethod);
ORKRequestMethod ORKRequestMethodTypeFromName(NSString *);

/**
 Cache policy for determining how to use ORKCache
 */
typedef enum {
    /**
     Never use the cache
     */
    ORKRequestCachePolicyNone = 0,
    /**
     Load from the cache when we are offline
     */
    ORKRequestCachePolicyLoadIfOffline = 1 << 0,
    /**
     Load from the cache if we encounter an error
     */
    ORKRequestCachePolicyLoadOnError = 1 << 1,
    /**
     Load from the cache if we have data stored and the server returns a 304
     (not modified) response
     */
    ORKRequestCachePolicyEtag = 1 << 2,
    /**
     Load from the cache if we have data stored
     */
    ORKRequestCachePolicyEnabled = 1 << 3,
    /**
     Load from the cache if we are within the timeout window
     */
    ORKRequestCachePolicyTimeout = 1 << 4,
    /**
     The default cache policy is etag and timeout support
     */
    ORKRequestCachePolicyDefault = ORKRequestCachePolicyEtag | ORKRequestCachePolicyTimeout
} ORKRequestCachePolicy;
...
typedef enum {
     ...
     */
    ORKRequestAuthenticationTypeNone = 0,
    /**
     Use NSURLConnection's HTTP AUTH auto-negotiation
     */
    ORKRequestAuthenticationTypeHTTP,
    /**
     ...
     */
    ORKRequestAuthenticationTypeHTTPBasic,
    /**
...
} ORKRequestAuthenticationType;

@class ORKRequest, ORKResponse, ORKRequestQueue, ORKReachabilityObserver;
@protocol ORKRequestDelegate, ORKConfigurationDelegate;

///-----------------------------------------------------------------------------
/// @name Block Declarations
///-----------------------------------------------------------------------------
typedef void(^ORKRequestDidLoadResponseBlock)(ORKResponse *response);
typedef void(^ORKRequestDidFailLoadWithErrorBlock)(NSError *error);

/**
 Models the request portion of an HTTP request/response cycle.
 */
@interface ORKRequest : NSObject {
    BOOL _sentSynchronously;
    NSURLConnection *_connection;
    id<ORKRequestDelegate> _delegate;
    NSTimer *_timeoutTimer;
    ORKRequestCachePolicy _cachePolicy;

    ORKRequestDidLoadResponseBlock _onDidLoadResponse;
    ORKRequestDidFailLoadWithErrorBlock _onDidFailLoadWithError;
}
...
/**
 Creates and returns a ORKRequest object initialized to load content from a
 provided URL.

 @param URL The remote URL to load
 @return An autoreleased ORKRequest object initialized with URL.
 */
+ (ORKRequest *)requestWithURL:(NSURL *)URL;

/**
 Initializes a ORKRequest object to load from a provided URL

 @param URL The remote URL to load
 @return An ORKRequest object initialized with URL.
 */
...
/**
 ...
 @bug **DEPRECATED** in v0.10.0: Use [ORKRequest requestWithURL:] instead
 @param URL The remote URL to load
 @param delegate The delegate that will handle the response callbacks.
 @return An autoreleased ORKRequest object initialized with URL.
 */
+ (ORKRequest *)requestWithURL:(NSURL *)URL delegate:(id)delegate DEPRECATED_ATTRIBUTE;

/**
 Initializes a ORKRequest object to load from a provided URL

 @bug **DEPRECATED** in v0.10.0: Use [ORKRequest initWithURL:] instead
 @param URL The remote URL to load
 @param delegate The delegate that will handle the response callbacks.
 @return An ORKRequest object initialized with URL.
 */
 ...
 **Default**: ORKRequestMethodGET
 */
@property (nonatomic, assign) ORKRequestMethod method;

/**
 Returns HTTP method as a string used for this request.

 This should be set through the method property using an ORKRequestMethod type.

 @see [ORKRequest method]
 */
 ...
 */
@property (nonatomic, retain) NSObject<ORKRequestSerializable> *params;
...
/**
 ...
 If the object implements the ORKRequestDelegate protocol, it will receive
 request lifecycle event messages.
 */
@property (nonatomic, assign) id<ORKRequestDelegate> delegate;
...
/**
 ...
 ORKClient and ORKObjectManager conform to the ORKConfigurationDelegate protocol.
...
/**
 ...
 @see [ORKRequestDelegate request:didLoadResponse:]
 */
@property (nonatomic, copy) ORKRequestDidLoadResponseBlock onDidLoadResponse;
 ...
 */
@property (nonatomic, copy) ORKRequestDidFailLoadWithErrorBlock onDidFailLoadWithError;
 ...
 **Default:** ORKRequestBackgroundPolicyCancel
 */
@property (nonatomic, assign) ORKRequestBackgroundPolicy backgroundPolicy;
...
/**
 ...
 Generally configured by the ORKClient instance that minted this request
 ...
 */
@property (nonatomic, retain) ORKRequestCache *cache;
 ...
 */
@property (nonatomic, assign) ORKRequestQueue *queue;
...
/**
 ...
 @return An ORKResponse object with the result of the request.
 */
...
/**
 ...
 However, it's good practice to cancel the ORKRequest and immediately set the
...
/**
 Resets the state of an ORKRequest so that it can be re-sent.
 */
- (void)reset;
...
/**
 ...
 @param error An NSError object containing the ORKRestKitError that triggered
 the callback.
 */
...
/**
 ...
 @param response An ORKResponse object with the result of the request.
 */
...
/**
 ...
 an ORKRequestConnectionTimeoutError via didFailLoadWithError:
 */
- (void)timeout;
...
/**
 ...
 Called by ORKResponse when the NSURLConnection begins receiving data.
 */
...
/**
 Lifecycle events for an ORKRequest object
 */
@protocol ORKRequestDelegate <NSObject>
@optional
...
/**
 ...
 @param request The ORKRequest object that is about to be sent.
 */
- (void)requestWillPrepareForSend:(ORKRequest *)request;

/**
 Sent when a request has received a response from the remote host.

 @param request The ORKRequest object that received a response.
 @param response The ORKResponse object for the HTTP response that was received.
 */
...
/**
 ...
 @param request The ORKRequest object that has begun loading.
 */
...
/**
 ...
 @param request The ORKRequest object that is handling the loading.
...
/**
 ...
 received from the remote site.
 */
- (void)request:(ORKRequest *)request didReceiveData:(NSInteger)bytesReceived totalBytesReceived:(NSInteger)totalBytesReceived totalBytesExpectedToReceive:(NSInteger)totalBytesExpectedToReceive;
...
/**
 ...
 @param request The ORKRequest object that was handling the loading.
 @param response The ORKResponse object containing the result of the request.
 */
...
/**
 ...
 @param request The ORKRequest object that was cancelled.
 */
...
/**
 ...
 @param request The ORKRequest object that timed out.
 */
