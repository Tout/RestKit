...
- (id)initWithBaseURL:(NSURL *)theBaseURL resourcePath:(NSString *)theResourcePath queryParameters:(NSDictionary *)theQueryParameters
{
    ...
    // So we unfortunately convert into an NSURL before going back into an NSString -> ORKURL
    NSURL *completeURL = [NSURL URLWithString:completePathWithQuery relativeToURL:theBaseURL];
    if (!completeURL) {
        ORKLogError(@"Failed to build ORKURL by appending resourcePath and query parameters '%@' to baseURL '%@'", theResourcePath, theBaseURL);
        [self release];
        return nil;
    }
...
{
    return [ORKURL URLWithBaseURL:self resourcePath:theResourcePath];
}

- (ORKURL *)URLByAppendingResourcePath:(NSString *)theResourcePath queryParameters:(NSDictionary *)theQueryParameters
{
    return [ORKURL URLWithBaseURL:self resourcePath:theResourcePath queryParameters:theQueryParameters];
}

- (ORKURL *)URLByAppendingQueryParameters:(NSDictionary *)theQueryParameters
{
    return [ORKURL URLWithBaseURL:self resourcePath:nil queryParameters:theQueryParameters];
}

- (ORKURL *)URLByReplacingResourcePath:(NSString *)newResourcePath
{
    return [ORKURL URLWithBaseURL:self.baseURL resourcePath:newResourcePath];
}
