...
#define ORKResponseIgnoreDelegateIfCancelled(...)                                   \
if (self.request && [self.request isCancelled]) {                                                  \
ORKLogDebug(@"%s: Ignoring NSURLConnection delegate message sent after cancel.", __PRETTY_FUNCTION__);     \
return __VA_ARGS__;                                                                \
}

@implementation ORKResponse
...
- (id)initWithRequest:(ORKRequest *)request
{
    self = [self init];
    if (self) {
        // We don't retain here as we're letting ORKRequestQueue manage
        // request ownership
        _request = request;
    }
...
- (id)initWithRequest:(ORKRequest *)request body:(NSData *)body headers:(NSDictionary *)headers
{
...
- (id)initWithSynchronousRequest:(ORKRequest *)request URLResponse:(NSHTTPURLResponse *)URLResponse body:(NSData *)body error:(NSError *)error
{
    self = [super init];
    if (self) {
        _request = request;
...
{
    ORKResponseIgnoreDelegateIfCancelled();
    ORKLogDebug(@"Received authentication challenge");
    ...
    if ([challenge previousFailureCount] == 0) {
        ...
        ORKLogWarning(@"Failed authentication challenge after %ld failures", (long)[challenge previousFailureCount]);
        [[challenge sender] cancelAuthenticationChallenge:challenge];
    }
}
...
{
    ORKResponseIgnoreDelegateIfCancelled(NO);
    ORKLogDebug(@"Asked if canAuthenticateAgainstProtectionSpace: with authenticationMethod = %@", [space authenticationMethod]);
    ...
    if (! hasCredentials) {
        ORKLogWarning(@"Received an authentication challenge without any credentials to satisfy the request.");
    }
...
- (NSURLRequest *)connection:(NSURLConnection *)connection willSendRequest:(NSURLRequest *)request redirectResponse:(NSURLResponse *)response
{
  if (nil == response || _request.followRedirect) {
    ORKLogDebug(@"Proceeding with request to %@", request);
    return request;
  } else {
    ORKLogDebug(@"Not following redirect to %@", request);
    return nil;
  }
}
...
{
    ORKResponseIgnoreDelegateIfCancelled();
    ORKLogDebug(@"NSHTTPURLResponse Status Code: %ld", (long)[response statusCode]);
    ORKLogDebug(@"Headers: %@", [response allHeaderFields]);
...
{
    ORKResponseIgnoreDelegateIfCancelled();
    ORKLogTrace(@"Read response body: %@", [self bodyAsString]);
...
{
    ORKResponseIgnoreDelegateIfCancelled(nil);
    ORKLogWarning(@"RestKit was asked to retransmit a new body stream for a request. Possible connection error or authentication challenge?");
    return nil;
}
...
// in connection:didReceiveResponse: to ensure that the ORKRequestDelegate
...
{
    id<ORKParser> parser = [[ORKParserRegistry sharedRegistry] parserForMIMEType:[self MIMEType]];
    if (! parser) {
        ORKLogWarning(@"Unable to parse response body: no parser registered for MIME Type '%@'", [self MIMEType]);
        return nil;
    }
    id object = [parser objectFromString:[self bodyAsString] error:error];
    if (object == nil) {
        if (error && *error) {
            ORKLogError(@"Unable to parse response body: %@", [*error localizedDescription]);
        }
        return nil;
    }
    return object;
...
- (NSURL *)URL
{
    if ([self wasLoadedFromCache]) {
        return [NSURL URLWithString:[_responseHeaders valueForKey:ORKRequestCacheURLHeadersKey]];
    }
...
{
    if ([self wasLoadedFromCache]) {
        return [_responseHeaders valueForKey:ORKRequestCacheMIMETypeHeadersKey];
    }
...
{
    if ([self wasLoadedFromCache]) {
        return [[_responseHeaders valueForKey:ORKRequestCacheStatusCodeHeadersKey] intValue];
    }
