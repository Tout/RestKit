...
@interface ORKOAuthClient () <ORKRequestDelegate>
@end

@implementation ORKOAuthClient
...
{
    ORKOAuthClient *client = [[[self alloc] initWithClientID:clientID secret:secret] autorelease];
    return client;
}
...
- (void)validateAuthorizationCode
{
    ...
    ORKRequest *theRequest = [ORKRequest requestWithURL:URL];
    theRequest.delegate = self;
    [theRequest setHTTPBodyString:httpBody];
    [theRequest setMethod:ORKRequestMethodPOST];
    [theRequest send];
}
...
- (void)request:(ORKRequest *)request didLoadResponse:(ORKResponse *)response
{
    ...
    if ([oauthResponse isKindOfClass:[NSDictionary class]]) {
        ...
        if (_accessToken) {
            ...
            // Invalid grant
            if (errorCode == ORKOAuthClientErrorInvalidGrant && [self.delegate respondsToSelector:@selector(OAuthClient:didFailWithInvalidGrantError:)]) {
                [self.delegate OAuthClient:self didFailWithInvalidGrantError:error];
            }

            // Unauthorized client
            if (errorCode == ORKOAuthClientErrorUnauthorizedClient && [self.delegate respondsToSelector:@selector(OAuthClient:didFailWithUnauthorizedClientError:)]) {
                [self.delegate OAuthClient:self didFailWithUnauthorizedClientError:error];
            }

            // Invalid client
            if (errorCode == ORKOAuthClientErrorInvalidClient && [self.delegate respondsToSelector:@selector(OAuthClient:didFailWithInvalidClientError:)]) {
                [self.delegate OAuthClient:self didFailWithInvalidClientError:error];
            }

            // Invalid request
            if (errorCode == ORKOAuthClientErrorInvalidRequest && [self.delegate respondsToSelector:@selector(OAuthClient:didFailWithInvalidRequestError:)]) {
                [self.delegate OAuthClient:self didFailWithInvalidRequestError:error];
            }

            // Unsupported grant type
            if (errorCode == ORKOAuthClientErrorUnsupportedGrantType && [self.delegate respondsToSelector:@selector(OAuthClient:didFailWithUnsupportedGrantTypeError:)]) {
                [self.delegate OAuthClient:self didFailWithUnsupportedGrantTypeError:error];
            }

            // Invalid scope
            if (errorCode == ORKOAuthClientErrorInvalidScope && [self.delegate respondsToSelector:@selector(OAuthClient:didFailWithInvalidScopeError:)]) {
...
- (void)request:(ORKRequest *)request didFailLoadWithError:(NSError *)error
{
    ...
    NSError *clientError = [NSError errorWithDomain:ORKErrorDomain code:ORKOAuthClientErrorRequestFailure userInfo:userInfo];
