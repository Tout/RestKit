...
/**
 * The multi-part boundary. See ORKParams.m
 */
extern NSString * const kORKStringBoundary;

@implementation ORKParamsAttachment
...
- (id)initWithName:(NSString *)name file:(NSString *)filePath
{
    ...
    if (self) {
        ...
        if (attributes) {
            ...
            ORKLogError(@"Encountered an error while determining file size: %@", error);
        }
    }
...
- (NSString *)MD5
{
    if (_body) {
        ...
        ORKLogWarning(@"Failed to generate MD5 for attachment: unknown data type.");
        return nil;
    }
}
