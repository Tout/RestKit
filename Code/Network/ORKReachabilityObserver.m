...
#define ORKLogComponent lcl_cRestKitNetworkReachability

@interface ORKReachabilityObserver (Private)
...
// Constants
NSString * const ORKReachabilityDidChangeNotification = @"ORKReachabilityDidChangeNotification";
NSString * const ORKReachabilityFlagsUserInfoKey = @"ORKReachabilityFlagsUserInfoKey";
NSString * const ORKReachabilityWasDeterminedNotification = @"ORKReachabilityWasDeterminedNotification";

static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void *info) {
    NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];

    ORKReachabilityObserver *observer = (ORKReachabilityObserver *)info;
...
@implementation ORKReachabilityObserver
...
+ (ORKReachabilityObserver *)reachabilityObserverForAddress:(const struct sockaddr *)address
{
    return [[[self alloc] initWithAddress:address] autorelease];
}

+ (ORKReachabilityObserver *)reachabilityObserverForInternetAddress:(in_addr_t)internetAddress
{
...
+ (ORKReachabilityObserver *)reachabilityObserverForInternet
{
    return [self reachabilityObserverForInternetAddress:INADDR_ANY];
}

+ (ORKReachabilityObserver *)reachabilityObserverForLocalWifi
{
    return [self reachabilityObserverForInternetAddress:IN_LINKLOCALNETNUM];
}

+ (ORKReachabilityObserver *)reachabilityObserverForHost:(NSString *)hostNameOrIPAddress
{
...
- (id)initWithAddress:(const struct sockaddr *)address
{
    ...
    if (self) {
        ...
        if (_reachabilityRef == NULL) {
            ORKLogWarning(@"Unable to initialize reachability reference");
            [self release];
            self = nil;
        } else {
            ...
            if (_monitoringLocalWiFi) {
                ORKLogInfo(@"Reachability observer initialized for Local Wifi");
            } else if (address->sa_len == sizeof(struct sockaddr_in) && address->sa_family == AF_INET) {
                ORKLogInfo(@"Reachability observer initialized with IP address: %@.", _host);
            }
...
- (id)initWithHost:(NSString *)hostNameOrIPAddress
{
    ...
    if (self) {
        ...
        ORKLogInfo(@"Reachability observer initialized with hostname %@", hostNameOrIPAddress);
        if (_reachabilityRef == NULL) {
            ORKLogWarning(@"Unable to initialize reachability reference");
            [self release];
            self = nil;
...
- (void)dealloc
{
    ORKLogTrace(@"Deallocating reachability observer %@", self);
...
- (NSString *)stringFromNetworkStatus:(ORKReachabilityNetworkStatus)status
{
    switch (status) {
        case ORKReachabilityIndeterminate:
            return @"ORKReachabilityIndeterminate";
            break;

        case ORKReachabilityNotReachable:
            return @"ORKReachabilityNotReachable";
            break;

        case ORKReachabilityReachableViaWiFi:
            return @"ORKReachabilityReachableViaWiFi";
            break;

        case ORKReachabilityReachableViaWWAN:
            return @"ORKReachabilityReachableViaWWAN";
            break;
...
- (ORKReachabilityNetworkStatus)networkStatus
{
    NSAssert(_reachabilityRef != NULL, @"currentNetworkStatus called with NULL reachabilityRef");
    ORKReachabilityNetworkStatus status = ORKReachabilityNotReachable;

    if (!self.reachabilityDetermined) {
        ORKLogTrace(@"Reachability observer %@ has not yet established reachability. networkStatus = %@", self, @"ORKReachabilityIndeterminate");
        return ORKReachabilityIndeterminate;
    }

    ORKLogTrace(@"Reachability Flags: %@\n", [self reachabilityFlagsDescription]);
    ...
    if (self.isMonitoringLocalWiFi) {
        if ((_reachabilityFlags & kSCNetworkReachabilityFlagsReachable) && (_reachabilityFlags & kSCNetworkReachabilityFlagsIsDirect)) {
            ...
            status = ORKReachabilityReachableViaWiFi;
        } else {
            // <-- NOT reachable OR NOT direct
            status = ORKReachabilityNotReachable;
        }
    } else {
        if ((_reachabilityFlags & kSCNetworkReachabilityFlagsReachable)) {
            ...
            if ((_reachabilityFlags & kSCNetworkReachabilityFlagsIsWWAN)) {
                ...
                status = ORKReachabilityReachableViaWWAN;
            } else {
#endif
                ...
                if ((_reachabilityFlags & kSCNetworkReachabilityFlagsConnectionOnTraffic) || (_reachabilityFlags & kSCNetworkReachabilityFlagsConnectionOnDemand)) {
                    ...
                    if ((_reachabilityFlags & kSCNetworkReachabilityFlagsInterventionRequired)) {
                        ...
                        status = (_reachabilityFlags & kSCNetworkReachabilityFlagsConnectionRequired) ? ORKReachabilityNotReachable : ORKReachabilityReachableViaWiFi;
                    } else {
    ...
    ORKLogTrace(@"Reachability observer %@ determined networkStatus = %@", self, [self stringFromNetworkStatus:status]);
    return status;
}
...
- (BOOL)isNetworkReachable
{
    ...
    BOOL reachable = (ORKReachabilityNotReachable != [self networkStatus]);
    ORKLogDebug(@"Reachability observer %@ determined isNetworkReachable = %d", self, reachable);
    return reachable;
}
...
- (BOOL)isConnectionRequired
{
    ...
    ORKLogDebug(@"Reachability observer %@ determined isConnectionRequired = %d", self, required);
    return required;
}
...
{
    [self validateIntrospection];
    return self.networkStatus == ORKReachabilityReachableViaWWAN;
}

- (BOOL)isReachableViaWiFi
{
    [self validateIntrospection];
    return self.networkStatus == ORKReachabilityReachableViaWiFi;
}
...
- (void)scheduleObserver
{
    ...
    ORKLogDebug(@"Scheduling reachability observer %@ in main dispatch queue", self);
    if (! SCNetworkReachabilitySetCallback(_reachabilityRef, ReachabilityCallback, &context)) {
        ORKLogWarning(@"%@: SCNetworkReachabilitySetCallback() failed: %s", self, SCErrorString(SCError()));
        return;
    }

    if (! SCNetworkReachabilitySetDispatchQueue(_reachabilityRef, dispatch_get_main_queue())) {
        ORKLogWarning("%@: SCNetworkReachabilitySetDispatchQueue() failed: %s", self, SCErrorString(SCError()));
        return;
    }
}
...
{
    if (_reachabilityRef) {
        ORKLogDebug(@"%@: Unscheduling reachability observer from main dispatch queue", self);
        if (! SCNetworkReachabilitySetDispatchQueue(_reachabilityRef, NULL)) {
            ORKLogWarning("%@: SCNetworkReachabilitySetDispatchQueue() failed: %s\n", self, SCErrorString(SCError()));
            return;
        }
    } else {
        ORKLogDebug(@"%@: Failed to unschedule reachability observer %@: reachability reference is nil.", self, _reachabilityRef);
    }
}
...
- (void)setReachabilityFlags:(SCNetworkReachabilityFlags)reachabilityFlags
{
    ...
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:[NSNumber numberWithUnsignedInt:reachabilityFlags] forKey:ORKReachabilityFlagsUserInfoKey];

   if (! self.reachabilityDetermined) {
       _reachabilityDetermined = YES;
       ORKLogInfo(@"Network availability has been determined for reachability observer %@", self);
       [[NSNotificationCenter defaultCenter] postNotificationName:ORKReachabilityWasDeterminedNotification object:self userInfo:userInfo];
   }

   // Post a notification to notify the client that the network reachability changed.
   [[NSNotificationCenter defaultCenter] postNotificationName:ORKReachabilityDidChangeNotification object:self userInfo:userInfo];
}
