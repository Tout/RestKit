...
/**
 ORKLogComponent defines the active component within any given portion of RestKit
 ...
 */
#define ORKLogComponent lcl_cRestKit
...
/**
 ...
 For example, in the ORKObjectMappingOperation class we would redefine the ORKLogComponent:
 ...
 Having defined the logging component, invoking the logger via:
    ...
    ORKLogInfo(@"This is my log message!");

 Would result in a log message similar to:

    I RestKit.ObjectMapping:ORKLog.h:42 This is my log message!
 ...
 */
#define ORKLogCritical(...)                                                              \
lcl_log(ORKLogComponent, lcl_vCritical, @"" __VA_ARGS__)

#define ORKLogError(...)                                                                 \
lcl_log(ORKLogComponent, lcl_vError, @"" __VA_ARGS__)

#define ORKLogWarning(...)                                                               \
lcl_log(ORKLogComponent, lcl_vWarning, @"" __VA_ARGS__)

#define ORKLogInfo(...)                                                                  \
lcl_log(ORKLogComponent, lcl_vInfo, @"" __VA_ARGS__)

#define ORKLogDebug(...)                                                                 \
lcl_log(ORKLogComponent, lcl_vDebug, @"" __VA_ARGS__)

#define ORKLogTrace(...)                                                                 \
lcl_log(ORKLogComponent, lcl_vTrace, @"" __VA_ARGS__)
 ...
 */
#define ORKLogLevelOff       lcl_vOff
#define ORKLogLevelCritical  lcl_vCritical
#define ORKLogLevelError     lcl_vError
#define ORKLogLevelWarning   lcl_vWarning
#define ORKLogLevelInfo      lcl_vInfo
#define ORKLogLevelDebug     lcl_vDebug
#define ORKLogLevelTrace     lcl_vTrace
...
/**
 ...
 Examples:
    ...
    ORKLogConfigureByName("RestKit/Network", ORKLogLevelDebug);

    // Log only critical messages from the Object Mapping component
    ORKLogConfigureByName("RestKit/ObjectMapping", ORKLogLevelCritical);
 */
#define ORKLogConfigureByName(name, level)                                               \
ORKLogInitialize();                                                                      \
lcl_configure_by_name(name, level);

/**
 Alias for configuring the LibComponentLogger logging component for the App. This
 enables the end-user of RestKit to leverage ORKLog() to log messages inside of
 their apps.
 */
#define ORKLogSetAppLoggingLevel(level)                                                  \
ORKLogInitialize();                                                                      \
 ...
 */
#define ORKLogToComponentWithLevelWhileExecutingBlock(_component, _level, _block)        \
    do {                                                                                \
 ...
 */
#define ORKLogSilenceComponentWhileExecutingBlock(component, _block)                      \
    ORKLogToComponentWithLevelWhileExecutingBlock(component, ORKLogLevelOff, _block)

/**
 Temporarily changes the logging level for the configured ORKLogComponent and executes the block. Any logging
 ...
 */
#define ORKLogWithLevelWhileExecutingBlock(_level, _block)                               \
    ORKLogToComponentWithLevelWhileExecutingBlock(ORKLogComponent, _level, _block)
...
/**
 ...
 After the block has finished execution, the logging level is restored to its previous state.
 */
#define ORKLogSilenceWhileExecutingBlock(_block)                                        \
    ORKLogToComponentWithLevelWhileExecutingBlock(ORKLogComponent, ORKLogLevelOff, _block)
...
/**
 ...
 You can override this setting by defining ORKLogLevelDefault as a pre-processor macro.
 */
#ifndef ORKLogLevelDefault
    #ifdef DEBUG
        #define ORKLogLevelDefault ORKLogLevelInfo
    #else
        #define ORKLogLevelDefault ORKLogLevelWarning
    #endif
#endif
...
/**
 ...
 Define an environment variable named ORKLogLevel.RestKit.Network and set its value to "Trace"
