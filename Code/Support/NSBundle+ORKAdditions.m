...
@implementation NSBundle (ORKAdditions)
...
+ (NSBundle *)restKitResourcesBundle
{
    ...
    if (! searchedForBundle) {
        ...
        if (! resourcesBundle) ORKLogWarning(@"Unable to find RestKitResources.bundle in your project. Did you forget to add it?");
...
- (NSData *)dataWithContentsOfResource:(NSString *)name withExtension:(NSString *)extension
{
    NSString *resourcePath = [self pathForResource:name ofType:extension];
    if (! resourcePath) {
        ORKLogWarning(@"%@ Failed to locate Resource with name '%@' and extension '%@': File Not Found.", self, resourcePath, extension);
        return nil;
    }
...
- (id)parsedObjectWithContentsOfResource:(NSString *)name withExtension:(NSString *)extension
{
    ...
    if (! parser) {
        ORKLogError(@"%@ Unable to parse Resource with name '%@' and extension '%@': failed to find parser registered to handle MIME Type '%@'", self, name, extension, MIMEType);
        return nil;
    }

    id object = [parser objectFromString:resourceContents error:&error];
    if (object == nil) {
        ORKLogCritical(@"%@ Failed to parse resource with name '%@' and extension '%@'. Error: %@", self, name, extension, [error localizedDescription]);
        return nil;
    }
