...
@interface ORKDotNetDateFormatter ()
- (NSString *)millisecondsFromString:(NSString *)string;
@end

@implementation ORKDotNetDateFormatter

+ (ORKDotNetDateFormatter *)dotNetDateFormatter
{
    return [ORKDotNetDateFormatter dotNetDateFormatterWithTimeZone:nil];
}

+ (ORKDotNetDateFormatter *)dotNetDateFormatterWithTimeZone:(NSTimeZone *)newTimeZone
{
    ORKDotNetDateFormatter *formatter = [[[ORKDotNetDateFormatter alloc] init] autorelease];
    if (newTimeZone)
...
- (NSDate *)dateFromString:(NSString *)string
{
    ...
    if (!milliseconds) {
        ORKLogError(@"Attempted to interpret an invalid .NET date string: %@", string);
        return nil;
    }
...
{
    if (!date) {
        ORKLogError(@"Attempted to represent an invalid date: %@", date);
        return nil;
    }
