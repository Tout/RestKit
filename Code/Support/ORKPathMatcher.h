...
/**
 ...
 of ORKRouter's function).  This makes it easier to implement ORKManagedObjectCache, and generate fetched
 ...
 @see ORKManagedObjectCache
 @see ORKMakePathWithObject
 @see ORKRouter
 */
@interface ORKPathMatcher : NSObject <NSCopying>

@property (copy, readonly) NSDictionary *queryParameters;

/**
 Creates an ORKPathMatcher starting from a resource path string.  This method should be followed by
 ...
 @return An instantiated ORKPathMatcher without an established pattern.
 */
+ (ORKPathMatcher *)matcherWithPath:(NSString *)pathString;
...
/**
 Creates an ORKPathMatcher starting from a pattern string.  This method should be followed by
 ...
 *NOTE 2 *- The pattern matcher supports KVM, so :key1.otherKey normally resolves as it would in any other KVM
 ...
 @param patternString The pattern to use for evaluating, such as /:entityName/:stateID/:chamber/
 @return An instantiated ORKPathMatcher with an established pattern.
 */
+ (ORKPathMatcher *)matcherWithPattern:(NSString *)patternString;
...
/**
 ...
 function to ORKMakePathWithObject (in fact it is a shortcut for this method).

 For example, given an 'article' object with an 'articleID' property value of 12345 ...

   ORKPathMatcher *matcher = [ORKPathMatcher matcherWithPattern:@"/articles/:articleID"];
...
/**
 ...
 ORKPathMatcher *matcher = [ORKPathMatcher matcherWithPattern:@"/articles/:articleID/:code"];
