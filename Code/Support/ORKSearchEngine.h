...
/**
 The ORKSearchable protocol declares a method for providing a searchable
 textual representation of content contained within the receiver. Searchable
 objects can be searched via instances of ORKSearchEngine.
 */
@protocol ORKSearchable
...
/**
 The mode (and/or) in which to search for tokenized text via an ORKSearchEngine instance.
 */
typedef enum {
    /**
...
/**
 An instance of ORKSearchEngine provides a simple interface for searching
 ...
 ORKSearchEngine is only suitable for searching a relatively small collection of in-memory
 objects that are not backed by Core Data (see ORKManagedObjectSearchEngine).

 @see ORKManagedObjectSearchEngine
 */
@interface ORKSearchEngine : NSObject
...
/**
 The type of matching to perform. Can be either ORKSearchModeAnd or ORKSearchModeOr.

 **Default**: ORKSearchModeOr
 */
@property (nonatomic, assign) ORKSearchMode mode;
...
/**
 Searches a collection of ORKSearchable objects for the given text using the configuration of the receiver
 ...
 conform to the ORKSearchable protocol.
 */
