...
BOOL ORKPathUsesParentheticalParameters(NSString *path);
NSString *ORKPathPatternFindAndReplaceParensWithColons(NSString *pattern);
NSString *ORKEncodeURLString(NSString *unencodedString);
...
NSString *ORKPathPatternFindAndReplaceParensWithColons(NSString *pattern) {
    if (ORKPathUsesParentheticalParameters(pattern)) {
        ORKLogWarning(@"Use of encapsulating parentheses for pattern parameters is deprecated.  Use a single colon instead. For example, instead of /group/(role)/(user) you should use /group/:role/:user");
...
{
    ORKPathMatcher *copy = [[[self class] allocWithZone:zone] init];
...
- (NSString *)pathFromObject:(id)object addingEscapes:(BOOL)addEscapes
{
    ...
    if (addEscapes)
        encoderBlock = ^NSString *(NSString *interpolatedString) {
            return ORKEncodeURLString(interpolatedString);
        };
